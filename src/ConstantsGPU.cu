...
std::pair<std::vector<Constants>, std::unique_ptr<Global>> SetupConstants(const std::vector<PrimeRecord>& q,
    ...
    // Hardcoded bounds of the constant tables every kernel strides over and of the per-GPU pools.
    // Exceeding one does not fail locally: it writes past the table into the neighbouring one and
    // the symptom surfaces at whatever synchronises next. Check while the numbers are cheap and
                                 ...
                                 ". Lower `depth`, or raise MAXP in src/ConstantsGPU.cuh and rebuild.");
    {
        int digits = 0;
        int maxDigit = -1;
        for (const auto& g : digitGPUid)
            digits = std::max(digits, (int)g.size());
        for (const auto& g : meta)
            for (const auto& l : g)
                maxDigit = std::max(maxDigit, l.digit);
        for (const auto& l : smeta)
            maxDigit = std::max(maxDigit, l.digit);
...

    {
        ...
        for (size_t i = 0; i < p.size(); ++i) {
            ...
            hC_.r64_shoup[hC_.L + i] = shoup_precomp((uint64_t)((((__uint128_t)1) << 64) % p[i].p), hC_.L + i, host_constants);
