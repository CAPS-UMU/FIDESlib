...
#define cudaGraphAddDependencies_compat(g, from, to, n) cudaGraphAddDependencies(g, from, to, nullptr, n)
#define cudaGraphNodeGetDependencies_compat(node, deps, n) cudaGraphNodeGetDependencies(node, deps, nullptr, n)
#define cudaGraphNodeGetDependentNodes_compat(node, deps, n) cudaGraphNodeGetDependentNodes(node, deps, nullptr, n)
#define cudaStreamGetCaptureInfo_compat(s, status, id, graph, deps, ndeps) cudaStreamGetCaptureInfo(s, status, id, graph, deps, nullptr, ndeps)
#define cudaStreamUpdateCaptureDependencies_compat(s, nodes, n, flags) cudaStreamUpdateCaptureDependencies(s, nodes, nullptr, n, flags)
#else
...
std::vector<cudaGraphNode_t> get_current_capture_dependencies(cudaStream_t s) {
    ...
    cudaError_t err = cudaStreamGetCaptureInfo_compat(s, &status, nullptr, nullptr, ((const cudaGraphNode_t**)&depNodes), &numDeps);
...
*/
    ...
    if (0) {
        ...
        err = cudaStreamGetCaptureInfo_compat(s,
            &status,
