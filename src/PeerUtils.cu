...
*/
    ...
    if (0)
    {
        ...
        err = cudaStreamGetCaptureInfo_compat(s, &status,
                                              ...
                                              &captured_graph, (const cudaGraphNode_t**)&deps_after_array,
                                              &numDeps_after);
...
__global__ void p2p_transfer_1d(const float* src, float* dst, size_t n)
    ...
    void transferKernel(float* src, float* dst, size_t elems, cudaStream_t s, int src_dev, int dst_dev,
                        size_t involved_sm)
    {
        // return;
