...
__device__ __forceinline__
    ...
    void
    backward_negacyclic_scale(char* buffer, const int primeid, T* psi, T* psi_shoup, const Global::Globals* Globals, const int logn) {
...
__device__ __forceinline__
    ...
    void
    INTT__(const Global::Globals* Globals,
        ...
        for (int i = 0; i < M; ++i) {
            T psi_aux[2];
            {
                ...
                for (int k = 0; k < 2; ++k) {
                    ...
                    if constexpr (algo == 3) {
                        psi_aux[k] = modmult<algo>(((T*)G_->inv_psi_no[primeid])[(lo_exp << 1) << (C_.logN - logN)], psi[hi_exp_br], primeid, psi_shoup[hi_exp_br]);
                    } else {
