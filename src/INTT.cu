...
namespace FIDESlib
{
    ...
    void backward_negacyclic_scale(
        char* buffer,
        const int primeid,
        T* psi,
        T* psi_shoup,
    ...
    void INTT__(const Global::Globals* Globals,
                ...
                const T* __restrict__ c0tilde)
    {
...

        {
            ...
            {
                mult_and_save_fusion<T, algo, M>(buffer, logBD, j, primeid, (T*)dat, (T*)dat2, (T*)c0, (T*)dat,
                                                 (T*)kska, (T*)kskb, (T*)c0, (T*)c0tilde);
            }
            ...
            {
                rotate_and_save_fusion<T, algo, M>(buffer, logBD, j, primeid, (T*)dat, (T*)c0, (T*)dat, (T*)kska,
                                                   (T*)kskb, (T*)c0);
            }
            else if constexpr (mode == INTT_SQUARE_AND_SAVE && !second)
            {
                square_and_save_fusion<T, algo, M>(buffer, logBD, j, primeid, (T*)dat, (T*)c0, (T*)dat, (T*)kska,
                                                   (T*)kskb, (T*)c0);
            }
            else
            {
            ...
            for (int i = 0; i < M; ++i)
            {
                T psi_aux[2];
                {
                    ...
                    for (int k = 0; k < 2; ++k)
                    {
                        ...
                        {
                            psi_aux[k] = modmult<algo>(psi[hi_exp_br],
                                                       ((T*)G_->inv_psi_no[primeid])[(lo_exp << 1) << (C_.logN - logN)],
                                                       primeid);
                        }
