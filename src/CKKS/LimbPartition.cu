...
void LimbPartition::generate(std::vector<LimbRecord>& records,
    ...
    for (int i = limbs_size; i <= pos; ++i) {
        ...
        if (r.type == U32) {
            if (buffer && buffer_aux) {
                limbs.emplace_back(Limb<uint32_t>(
                    cc, (uint32_t*)buffer, 2 * offset, id, USE_PARTITION_STREAM ? s : records.at(i).stream, r.id, (uint32_t*)buffer_aux, 2 * offset_aux, num_elems));
                offset += cc.N;
                offset_aux += cc.N;
            } else if (buffer) {
        ...
        if (r.type == U64) {
            if (buffer && buffer_aux) {
                limbs.emplace_back(Limb<uint64_t>(cc, buffer, offset, id, USE_PARTITION_STREAM ? s : records.at(i).stream, r.id, buffer_aux, offset_aux, num_elems));
                offset += cc.N;
                offset_aux += cc.N;
            } else if (buffer) {
    ...
    if (logN >= NTT_1D_LOG_THRESHOLD) {
        ...
        for (int i = 0; i < limbsize; i += batch) {
            ...
            INTT_1D_<algo, INTT_NONE><<<dim3{ 1u, num_limbs }, blockDimFirst, bytesFirst, STREAM(limb.at(i)).ptr()>>>(
                getGlobals(), limbptr.data + i, primeid_init + i, limbptr.data + i);
        }
    }
}
...
void LimbPartition::add(const LimbPartition& p, const bool exta, const bool extb, int slots) {
    ...
    for (int i = 0; i < limbsize; i += cc.batch) {
        ...
        if (exta == extb) {
            add_<<<dim3{ (uint32_t)GetGridDimX(), num_limbs }, GetBlockDimX(), 0, STREAM(limb[i]).ptr()>>>(limbptr.data + i, p.limbptr.data + i, PARTITION(id, i), 2 * slots);
        } else if (exta) {
            add_scale_p_b_<<<dim3{ (uint32_t)GetGridDimX(), num_limbs }, GetBlockDimX(), 0, STREAM(limb[i]).ptr()>>>(
                limbptr.data + i, p.limbptr.data + i, PARTITION(id, i), 2 * slots);
        } else if (extb) {
    ...
    if (exta || extb) {
        if (exta && !extb) {
            ...
            for (size_t i = start; i < static_cast<size_t>(start + num_limbs); i += cc.batch) {
                ...
                {
                    assert(2 * slots == cc.N);
                    copy_<<<dim3{ (uint32_t)GetGridDimX(), size }, GetBlockDimX(), 0, STREAM(SPECIALlimb[i]).ptr()>>>(p.SPECIALlimbptr.data + i, SPECIALlimbptr.data + i);
            ...
            for (size_t i = start; i < static_cast<size_t>(start + num_limbs); i += cc.batch) {
                ...
                uint32_t size = std::min((int)start + num_limbs - (int)i, cc.batch);
                {
                    add_<<<dim3{ (uint32_t)GetGridDimX(), size }, GetBlockDimX(), 0, STREAM(SPECIALlimb[i]).ptr()>>>(
                        SPECIALlimbptr.data + i, p.SPECIALlimbptr.data + i, SPECIAL(id, i), 2 * slots);
                    // TODO: have to check if Limbpartition comes from a plaintext, where extension limbs are mapped differently
                }
            }
            for (size_t i = start; i < static_cast<size_t>(start + num_limbs); i += cc.batch) {
...
void LimbPartition::sub(const LimbPartition& p, int slots) {
    ...
    for (int i = 0; i < limbsize; i += cc.batch) {
        STREAM(limb[i]).wait(s);
        uint32_t num_limbs = std::min((int)limbsize - i, cc.batch);
        sub_<<<dim3{ (uint32_t)GetGridDimX(), num_limbs }, GetBlockDimX(), 0, STREAM(limb[i]).ptr()>>>(limbptr.data + i, p.limbptr.data + i, PARTITION(id, i), 2 * slots);
    }
    for (int i = 0; i < limbsize; i += cc.batch) {
        s.wait(STREAM(limb[i]));
    }
...
void LimbPartition::multElement(const LimbPartition& p, int slots) {
    ...
    for (int i = 0; i < limbsize; i += cc.batch) {
        ...
        Mult_<<<dim3{ (uint32_t)GetGridDimX(), num_limbs }, GetBlockDimX(), 0, STREAM(limb[i]).ptr()>>>(
            limbptr.data + i, limbptr.data + i, p.limbptr.data + i, PARTITION(id, i), 2 * slots);
    }
    for (int i = 0; i < limbsize; i += cc.batch) {
        s.wait(STREAM(limb[i]));
    }
...
void LimbPartition::multElement(const LimbPartition& partition1, const LimbPartition& partition2, int slots_p2) {
    ...
    for (int i = 0; i < limbsize; i += cc.batch) {
        ...
        uint32_t num_limbs = std::min((int)limbsize - i, cc.batch);
        Mult_<<<dim3{ (uint32_t)GetGridDimX(), num_limbs }, GetBlockDimX(), 0, STREAM(limb[i]).ptr()>>>(
            (void**)limbptr.data + i, (void**)partition1.limbptr.data + i, (void**)partition2.limbptr.data + i, PARTITION(id, i), slots_p2);
    }
    for (int i = 0; i < limbsize; i += cc.batch) {
        s.wait(STREAM(limb[i]));
    }
...
void LimbPartition::rescale(int slots) {
    ...
    if (logN >= NTT_1D_LOG_THRESHOLD && aux_size == 0) {
        ...
        if (limbsize > 0) {
            ...
            {
                NTT_<false, algo, NTT_RESCALE><<<dim3{ NTT_grid_dim_X<M, false>(logN), static_cast<unsigned int>(limbsize - 1) }, blockDimFirst, bytesFirst, s.ptr()>>>(
                    getGlobals(), limbptr.data + limbsize - 1, PARTITION(id, 0), auxLimbs.limbptr.data, nullptr /*limbptr.data*/, -1, PRIMEID(top));

                NTT_<true, algo, NTT_RESCALE><<<dim3{ INTT_grid_dim_X<M, true>(logN), static_cast<unsigned int>(limbsize - 1) }, blockDimSecond, bytesSecond, s.ptr()>>>(
                    getGlobals(), auxLimbs.limbptr.data, PARTITION(id, 0), limbptr.data, nullptr, -1, PRIMEID(top));
            }
        }
        auxLimbs.s.wait(s);
    } else {
...
void LimbPartition::dotKSK(const LimbPartition& src, const LimbPartition& ksk, const bool inplace, const LimbPartition* limbsrc) {
    ...
    if constexpr (0) {
        ...
        for (uint32_t i = 0; i < DECOMPmeta.size(); ++i) {
            ...
            Mult_<<<{ (uint32_t)GetGridDimX(), (uint32_t)size }, GetBlockDimX(), 0, s.ptr()>>>(inplace ? auxptr.data + start : limbptr.data + start,
        ...
        for (uint32_t i = 0; i < DIGITmeta.size(); ++i) {
            ...
            if (start > 0) {
                ...
                addMult_<<<{ (uint32_t)GetGridDimX(), (uint32_t)size }, GetBlockDimX(), 0, s.ptr()>>>(
                    inplace ? auxptr.data : limbptr.data, ksk.DIGITlimbptr[i].data + special, src.DIGITlimbptr[i].data + special, 0, cc.N);
            }
            start += DECOMPmeta[i].size();
            if (start < limbsize) {
        ...
        for (uint32_t i = 0; i < DIGITmeta.size(); ++i) {
            ...
            if (i == 0) {
                Mult_<<<{ (uint32_t)GetGridDimX(), (uint32_t)special }, GetBlockDimX(), 0, s.ptr()>>>(
                    inplace ? SPECIALauxptr.data : SPECIALlimbptr.data, ksk.DIGITlimbptr[i].data, src.DIGITlimbptr[i].data, SPECIAL(id, 0), cc.N / 2);
            } else {
                addMult_<<<{ (uint32_t)GetGridDimX(), (uint32_t)special }, GetBlockDimX(), 0, s.ptr()>>>(
                    inplace ? SPECIALauxptr.data : SPECIALlimbptr.data, ksk.DIGITlimbptr[i].data, src.DIGITlimbptr[i].data, SPECIAL(id, 0), cc.N);
            }
        }
    }
...
size_t LimbPartition::getLimbSize(int level) const {
    size_t size = 0;
    if (level == cc.L + 1) {
...
void LimbPartition::automorph(const int index, const int br, LimbPartition* src, const bool ext, int num_elems) {
    ...
    for (int i = 0; i < limbsize; i += cc.batch) {
        ...
        automorph_multi_<<<dim3{ (uint32_t)GetGridDimX(), num_limbs }, GetBlockDimX(), 0, STREAM(limb[i]).ptr()>>>(
            src ? src->limbptr.data + i : limbptr.data + i, src ? limbptr.data + i : auxptr.data + i, index, br, PARTITION(id, i), logn);
    }
    if (ext) {
        int start = cc.splitSpecialMeta.at(id).at(0).id - (cc.L + 1);
        int num_limbs = cc.splitSpecialMeta.at(id).size();
        ...
        for (int32_t i = start; i < start + num_limbs; i += 1 /*cc.batch*/) {
            ...
            automorph_multi_<<<dim3{ (uint32_t)GetGridDimX(), size },
                GetBlockDimX(),
                0,
                STREAM(SPECIALlimb[i - (SPECIALlimb.size() < SPECIALmeta.size()) * start]).ptr()>>>(src ?
...
void LimbPartition::multScalar(std::vector<uint64_t>& vector) {
    ...
    for (int i = 0; i < limbsize; i += cc.batch) {
        STREAM(limb[i]).wait(s);
        uint32_t num_limbs = std::min((int)limbsize - i, cc.batch);
        Scalar_mult_<ALGO_BARRETT>
            <<<dim3{ (uint32_t)GetGridDimX(), num_limbs }, GetBlockDimX(), 0, STREAM(limb[i]).ptr()>>>(limbptr.data + i, elems, PARTITION(id, i), nullptr);
    }
    if (*level == cc.L + 1 && SPECIALmeta.size() > 0 && SPECIALmeta.at(0).id == cc.L + 1) {
        Scalar_mult_<ALGO_BARRETT>
            <<<dim3{ (uint32_t)GetGridDimX(), 1 }, GetBlockDimX(), 0, STREAM(SPECIALlimb[0]).ptr()>>>(SPECIALlimbptr.data, elems, SPECIAL(id, 0), nullptr);
    ...
    for (int i = 0; i < limbsize; i += cc.batch) {
        ...
        if (!ext_a && ext_b) {
            addScaleB_<<<dim3{ (uint32_t)GetGridDimX(), num_limbs }, GetBlockDimX(), 0, STREAM(limb[i]).ptr()>>>(
                limbptr.data + i, a.limbptr.data + i, b.limbptr.data + i, PARTITION(id, i), 2 * slots_b);
        } else if (!ext_b && ext_a) {
    ...
    if (ext_a || ext_b) {
        ...
        for (int32_t i = start; i < start + num_limbs; i += cc.batch) {
            ...
            if (!ext_a && ext_b) {
                ...
                copy_<<<dim3{ (uint32_t)GetGridDimX(), size }, GetBlockDimX(), 0, STREAM(SPECIALlimb[i]).ptr()>>>(b.SPECIALlimbptr.data + i, SPECIALlimbptr.data + i);
                // TODO: have to check if Limbpartition comes from a plaintext, where extension limbs are mapped differently
            } else if (!ext_b && ext_a) {
                copy_<<<dim3{ (uint32_t)GetGridDimX(), size }, GetBlockDimX(), 0, STREAM(SPECIALlimb[i]).ptr()>>>(a.SPECIALlimbptr.data + i, SPECIALlimbptr.data + i);
                // TODO: have to check if Limbpartition comes from a plaintext, where extension limbs are mapped differently
            } else {
                add_<<<dim3{ (uint32_t)GetGridDimX(), size }, GetBlockDimX(), 0, STREAM(SPECIALlimb[i]).ptr()>>>(
                    SPECIALlimbptr.data + i, a.SPECIALlimbptr.data + i, b.SPECIALlimbptr.data + i, SPECIAL(id, i), 2 * slots_b);
                // TODO: have to check if Limbpartition comes from a plaintext, where extension limbs are mapped differently
            }
        }
        for (int32_t i = start; i < start + num_limbs; i += cc.batch) {
...
void LimbPartition::squareElement(const LimbPartition& p) {
    ...
    for (int i = 0; i < size; i += cc.batch) {
        ...
        square_<<<dim3{ (uint32_t)GetGridDimX(), num_limbs }, GetBlockDimX(), 0, STREAM(limb[i]).ptr()>>>(limbptr.data + i, p.limbptr.data + i, PARTITION(id, i));
    }
    for (int i = 0; i < size; i += cc.batch) {
        s.wait(STREAM(limb[i]));
    }
...
void LimbPartition::addMult(const LimbPartition& a, const LimbPartition& b, int slots) {
    ...
    for (int i = 0; i < limbsize; i += cc.batch) {
        STREAM(limb[i]).wait(s);
        uint32_t num_limbs = std::min((int)limbsize - i, cc.batch);
        addMult_<<<dim3{ (uint32_t)GetGridDimX(), num_limbs }, GetBlockDimX(), 0, STREAM(limb[i]).ptr()>>>(
            limbptr.data + i, a.limbptr.data + i, b.limbptr.data + i, PARTITION(id, i), 2 * slots);
    }
    for (int i = 0; i < limbsize; i += cc.batch) {
        s.wait(STREAM(limb[i]));
    }
...
void LimbPartition::evalLinearWSum(uint32_t n, std::vector<const LimbPartition*> ps, std::vector<uint64_t>& weights, bool with_bias) {
    const int limbsize = getLimbSize(*level);
    cudaSetDevice(device);
    ...
    if (!limb.empty() && limbsize > 0) {
        if (with_bias) {
            ...
            eval_linear_w_sum_<<<dim3{ (uint32_t)GetGridDimX(), (uint32_t)limbsize }, GetBlockDimX(), 0, s.ptr()>>>(n, limbptr.data, d_psptr, elems, PARTITION(id, 0));
        }
    }
    cudaFreeAsync(elems, s.ptr());
...
void LimbPartition::evalLinearWSumMultiple(int max_n,
    ...
    int limbsize = 0;
    for (auto& i : out)
        limbsize = std::max(limbsize, (int)i->getLimbSize(*i->level));
    ...
    uint64_t* elems;
    cudaMallocAsync(&elems, elem.size() * sizeof(uint64_t), c.s.ptr());
    cudaMemcpyAsync(elems, elem.data(), elem.size() * sizeof(uint64_t), cudaMemcpyDefault, c.s.ptr());
    std::vector<void**> inptr(max_n + 2 * out.size(), nullptr);
    void*** outptr = inptr.data() + max_n;
    int* target_limbsize = (int*)(inptr.data() + max_n + out.size());
    for (int32_t i = 0; i < max_n; ++i) {
        inptr[i] = in[i]->limbptr.data;
        assert(in[i]->limb.size() >= limbsize);
    }
    for (uint32_t i = 0; i < out.size(); ++i) {
        outptr[i] = out[i]->limbptr.data;
        target_limbsize[i] = out[i]->getLimbSize(*out[i]->level);
    }
    void*** d_inptr;
    cudaMallocAsync(&d_inptr, inptr.size() * sizeof(void**), c.s.ptr());
    void*** d_outptr = d_inptr + max_n;
    ...
    if (limbsize > 0) {
        eval_batched_linear_w_sum_with_bias_<<<dim3{ (uint32_t)c.GetGridDimX(), (uint32_t)limbsize }, c.GetBlockDimX(), c.GetBlockDimX() * sizeof(uint64_t) * out.size(), c.s.ptr()>>>(
            max_n, out.size(), d_outptr, d_inptr, d_target_limbsize, elems, PARTITION(c.id, 0), isC1);
    }
...
void LimbPartition::generatePartialSpecialLimb(int slots) {
    ...
    if (SPECIALlimb.size() == 0 && cc.splitSpecialMeta.at(id).size() > 0 /*&& bufferSPECIAL == nullptr*/) {
        ...
        generate(cc.splitSpecialMeta.at(id), SPECIALlimb, SPECIALlimbptr, (int)cc.splitSpecialMeta.at(id).size() - 1, nullptr, nullptr, 0, nullptr, 0, false, 2 * slots);
        // for (auto& l : SPECIALlimb)
        //     STREAM(l).wait(s);
    }
}
...
void LimbPartition::dotProductPt(LimbPartition& c1,
    ...
    for (int32_t i = 0; i < limbsize; i += cc.batch) {
        ...
        dotProductPt_<<<dim3{ (uint32_t)GetGridDimX(), (uint32_t)size }, GetBlockDimX(), 0, STREAM(limb[i]).ptr()>>>(
            limbptr.data, c1.limbptr.data, data.data, i, PARTITION(id, i), n, 2 * slots);
    }
    ...
    if (ext) {
        ...
        for (uint32_t i = start; i < start + num_limbs; i += cc.batch) {
            ...
            dotProductPt_<<<dim3{ (uint32_t)GetGridDimX(), size }, GetBlockDimX(), 0, STREAM(SPECIALlimb[i]).ptr()>>>(
                SPECIALlimbptr.data + start, c1.SPECIALlimbptr.data + start, data.data + 3 * n, i - start, SPECIAL(id, i), n, 2 * slots);
        }
        for (uint32_t i = start; i < start + num_limbs; i += cc.batch) {
            STREAM(SPECIALlimb[i]).wait(s);
        }
