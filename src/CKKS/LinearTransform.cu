                    ...
                    red_n + j])
                {
                    ...
                    result[i]->slots =
                        std::max(result[i]->slots,
                                 b[(i % pt_different_stride + (i / (pt_reuse_stride * pt_different_stride)) *
                                     pt_different_stride) * red_n + j]->slots);
                }
            }
        }
    }
...
std::vector<int> FIDESlib::CKKS::GetConvolutionTransformRotationIndices(int rowSize, int bStep, int stride,
                                                                        uint32_t gStep)
{
    std::vector<int> res;
    // Internal block size for DotProductPtInternal
...
*/
    ...
    result.NoiseFactor = cc.param.ScalingFactorRealBig[result.getLevel() - 1] * cc.param.ModReduceFactor[result.
        getLevel()];
...
std::vector<int> FIDESlib::CKKS::GetLinearTransformPlaintextRotationIndices(
    int rowSize, int bStep, int stride, int offset)
{
