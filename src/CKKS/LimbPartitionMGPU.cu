        ...
        if (cc.limbGPUid[*level].x == static_cast<uint32_t>(id))
        {
            if (limb.size() > cc.limbGPUid[*level].y && PRIMEID(limb[cc.limbGPUid[*level].y]) == *level)
            {
                ...
                // SWITCH(top, INTT<ALGO_SHOUP>());
                {
                    ...
                    const int bytesFirst = NTT_shmem < M, uint64_t, algo
                    >
                    (blockDimFirst.x);
                    const int bytesSecond = NTT_shmem < M, uint64_t, algo
                    >
                    (blockDimSecond.x);
                    ...
                    for (int i = limbsize - 1; i < limbsize; i += cc.batch)
                    {
                        ...
                        INTT_ < false, algo, INTT_NONE >  << < dim3{INTT_grid_dim_X < M, false > (logN), num_limbs},
                            blockDimFirst, bytesFirst, stream.ptr()
                            >
                            >
                            >
                            (
                        ...
                        INTT_ < true, algo, INTT_NONE >  << < dim3{INTT_grid_dim_X < M, true > (logN), num_limbs},
                            blockDimSecond, bytesSecond, stream.ptr()
                            >
                            >
                            >
                            (
...
*/
        ...
        {
            {
                ...
                for (int i = 0; i < size; i += batch)
                {
                    ...
                    NTT_ < false, algo, mode >  << < dim3{NTT_grid_dim_X < M, false > (logN), num_limbs}, blockDimFirst,
                    ...
                    NTT_ < true, algo, mode >  << < dim3{NTT_grid_dim_X < M, false > (logN), num_limbs}, blockDimSecond,
                        bytesSecond, s.ptr()
                        >
                        >
                        >
    ...
    void LimbPartition::doubleRescaleMGPU(LimbPartition& partition)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            {
                if (part[i]->limb.size() > cc.limbGPUid[*part[i]->level].y && PRIMEID(
                        part[i]->limb[cc.limbGPUid[*part[i]->level].y]) == *part[i]->level ||
                    (*part[i]->level == cc.L + 1 && part[i]->SPECIALlimb.size() > 0 && PRIMEID(
                            part[i]->SPECIALlimb.at(cc.limbGPUid[*part[i]->level].y)) == *part[i]
                        ->level))
                {
                    LimbImpl& top = (*part[i]->level == cc.L + 1)
                                        ? part[i]->SPECIALlimb.at(0)
                                        : part[i]->limb.at(limbsize - 1);
                    if (1)
                    {
                        ...
                        for (int i_ = limbsize - 1; i_ < limbsize; i_ += cc.batch)
                        {
                            ...
                            INTT_ < false, algo, INTT_NONE >  << < dim3{cc.N / (blockDimFirst.x * M * 2), num_limbs},
                                blockDimFirst, bytesFirst, stream[i]->ptr()
                                >
                                >
                                >
                                (
                                    getGlobals(),
                                    *part[i]->level == cc.L + 1
                                        ? part[i]->SPECIALlimbptr.data + 0
                                        : part[i]->limbptr.data + start + i_,
...
*/
                ...
                {
                    std::cout << "Cant find the top limb!!!" << part[i]->id << " " << *part[i]->level << " " << cc.
                        limbGPUid[*part[i]->level].y << " "
        ...
        for (int j = 0; j < 2; ++j)
        {
            // stream[j]->wait(part[j]->s);
            {
                ...
                >
                (blockDimSecond.x);
                const int size = getLimbSize(
                    *part[j]->level - 1 - (*part[j]->level == cc.L + 1 && cc.rescaleTechnique ==
                        CKKS::FLEXIBLEAUTOEXT));
                const int batch = cc.batch;
                const NTT_MODE mode = NTT_RESCALE;
                ...
                for (int i = 0; i < size; i += batch)
                {
                    ...
                    NTT_ < false, algo, mode > 
                        << < dim3{cc.N / (blockDimFirst.x * M * 2), num_limbs}, blockDimFirst, bytesFirst, id == top_gpu
                                ? part[j]->s.ptr()
                                : stream[j]->ptr()
                                >
                                >
                    ...
                    NTT_ < true, algo, mode >  << < dim3{cc.N / (blockDimSecond.x * M * 2), num_limbs}, blockDimSecond,
                        bytesSecond, part[j]->s.ptr()
                        >
                        >
                        >
                        (
	...
	*/
                                         ...
                                         bool c0_modup)
    {
        ...
        if constexpr (1)
        {
            ...
            hoistedRotateDotKSK_2_<<<dim3{(uint32_t)cc.N / 128, (uint32_t)num_special + num_limbs}, 128, sizeof(
                uint64_t) * 128 * i, s.ptr()>>>(
    ...
    void LimbPartition::modup_ksk_moddown_mgpu(LimbPartition& c0,
                                               ...
                                               std::vector<std::vector<std::vector<std::pair<
                                                   uint64_t, TimelineSemaphore*>>>>& signal,
                                               ...
                                               const std::vector<Stream*>& external_s0)
    {
        ...
        const int digits_per_it = MEMCPY_PEER ? 1 : 1 /*num_d*/;
        // cc.logN <= 15 ? num_d : cc.logN == 16 ? std::max((num_d + 1) / 2, 1) : 1;
...
*/
        if (GRAPH_CAPTURE)
        {
            if ((!MEMCPY_PEER || (MEMCPY_PEER && id == 0)))
            {
                if (exec_old != map_exec.end())
                {
                    ...
                    if (graph_data.buffKeyA == ksk_a.uid && graph_data.buffKeyB == ksk_b.uid && graph_data.buffAux1 ==
                        auxLimbs1.uid &&
                        graph_data.buffAux2 == auxLimbs2.uid && graph_data.buffC0 == c0.uid && graph_data.buffC1 == c1.
                        uid)
                    {
        ...
        for (int d = 0; d < num_d; d += digits_per_it)
        {
            ...
            if constexpr (PRINT)
                if (SELECT)
                {
                    std::cout << "GPU " << id << " for digits " << d << ":" << d + digits_per_it << " INTT " << size_d
                        << " limbs starting at limb " << start_d <<
                        std::endl;
                }
            ...
            if (size_d > 0)
            {
...

                {
                    ...
                    INTT_ < false, algo, INTT_NONE >  << < dim3{cc.N / (blockDimFirst.x * M * 2), size_d}, blockDimFirst
                        , bytesFirst, stream.ptr()
                        >
                        >
                        >
                    ...
                    INTT_ < true, algo, INTT_NONE >  << < dim3{cc.N / (blockDimSecond.x * M * 2), size_d},
                        blockDimSecond, bytesSecond, stream.ptr()
                        >
                        >
                        >
                        ...
                        }
                        {
                            ...
                            for (size_t i = 0; i < cc.GPUid.size(); ++i)
                            {
                                ...
                                while (start_d_i < limb_size_i && cc.meta[i][start_d_i].digit < d)
                                    start_d_i++;
                                uint32_t size_d_i = 0;
                                while (start_d_i + size_d_i < limb_size_i && cc.meta[i][start_d_i + size_d_i].digit < d
                                    + ds)
                                    size_d_i++;
                                if constexpr (PRINT)
                                    if (SELECT)
                                            ...
                                            if (j != i)
                                            {
                                                ...
                                                if (0 && GRAPH_CAPTURE)
                                                {
                                                    ...
                                                    transferKernel((float*)(bufferGATHER + cc.N * (start + start_d_i)),
                                                                   (float*)(bufferGather_[j] + cc.N * (start +
                                                                       start_d_i)),
            ...
            for (int d_ = d; d_ < d + ds; ++d_)
            {
                    ...
                    std::cout << "/** NTT special limbs */" << std::endl;
                {
                    ...
                    if (size > 0)
                    {
                        ...
                        NTT_ < true, algo, NTT_NONE >  << < dim3{cc.N / (blockDimSecond.x * M * 2), size},
                            blockDimSecond, bytesSecond, stream1.ptr()
                            >
                            >
                            >
                            (
            ...
            if (moddown)
            {
                    ...
                    std::cout << "/** INTT specials*/" << std::endl;
                {
                    ...
                    if (limbs > 0)
                    {
                        ...
                        INTT_ < false, algo, INTT_NONE >  << < dim3{cc.N / (blockDimFirst.x * M * 2), limbs},
                            blockDimFirst, bytesFirst, stream.ptr()
                            >
                            >
                            >
                        ...
                        INTT_ < true, algo, INTT_NONE >  << < dim3{cc.N / (blockDimSecond.x * M * 2), limbs},
                            blockDimSecond, bytesSecond, stream.ptr()
                            >
                            >
                            >
                        ...
                        // for (int i_ = 0; i_ < 2; ++i_)
                        {
                            ...
                            for (size_t i = 0; i < cc.splitSpecialMeta.size(); ++i)
                            {
                                ...
                                if (static_cast<uint32_t>(id) == i)
                                {
                                    if (num_limbs > 0)
                                    {
                                        uint64_t* ptr = bufferSpecial_[i] + (cc.splitSpecialMeta.at(i).at(0).id -
                                            SPECIALmeta.at(0).id) * cc.N;

                                        for (uint32_t j = 0; j < cc.GPUid.size(); ++j)
                                        {
                                            if (j != i)
                                            {
                                                Stream& stream_ = cc.digitStreamForMemcpyPeer.at(cc.dnum > 1 ? i_ : 0).
                                                                     at(i).at(j);
                                                stream_.wait(stream);
                                                ...
                                                if (0 && GRAPH_CAPTURE)
                                                {
                                                    ...
                                                    transferKernel((float*)(ptr),
                                                                   (float*)(bufferSpecial_[j] + (cc.splitSpecialMeta.
                                                                       at(i).at(0).id - SPECIALmeta.at(0).id) * cc.N),
                                    ...
                                    for (uint32_t j = 0; j < cc.GPUid.size(); ++j)
                                    {
                                        if (j == static_cast<uint32_t>(id))
                                        {
                                            if (i != j)
                                            {
                                                ...
                                                {
                                                    Stream& stream_ = cc.digitStreamForMemcpyPeer.at(
                                                        cc.dnum > 1 ? i_ : 0).at(i).at(j);
                                                    stream.wait(stream_);
                                                }
                                            }
                                        }
                    ...
                    else
                    {
                        // for (int i_ = 0; i_ < 2; ++i_)
                        {
                            ...
                            for (size_t j = 0; j < cc.splitSpecialMeta.size(); ++j)
                            {
                                ...
                                if (num_limbs > 0)
                                {
                                    uint64_t* ptr = bufferSpecial_ + (cc.splitSpecialMeta.at(j).at(0).id - SPECIALmeta.
                                        at(0).id) * cc.N;
                                    NCCLCHECK(ncclBroadcast(ptr, ptr, cc.N * num_limbs, ncclUint64, (int)j, rank,
                                                            stream.ptr()));
                                }
                            }
                            NCCLCHECK(ncclGroupEnd());
        ...
        if (moddown)
        {
            for (int i = 0; i < 2; ++i)
            {
                ...
                if (limb_size > 0)
                {
                    ...
                    {
                        NTT_ < false, algo, NTT_MODDOWN >  << < dim3{cc.N / (blockDimFirst.x * M * 2), limb_size},
                            blockDimFirst, bytesFirst, stream.ptr()
                            >
                            >
                            >
                        ...
                        NTT_ < true, algo, NTT_MODDOWN >  << < dim3{cc.N / (blockDimSecond.x * M * 2), limb_size},
                            blockDimSecond, bytesSecond, stream.ptr()
                            >
                            >
                            >
        ...
        {
            {
                ...
                if (!ok)
                {
                    ...
                    // cudaDeviceSynchronize();
                    // CudaCheckErrorModNoSync;
                    cudaGraphInstantiateWithFlags(&exec, exec_old->second.first,
                                  ...
                                  std::vector<Stream*>& external_s)
    {
        ...
        while (limb_size < meta.size() && meta[limb_size].id <= *level)
            limb_size++;
        const int digits_per_it = MEMCPY_PEER ? 1 : 1 /*dnum*/;
        // cc.logN <= 15 ? num_d : cc.logN == 16 ? std::max((num_d + 1) / 2, 1) : 1;
        s.wait(c0.s);
        if (GRAPH_CAPTURE)
        {
            ...
            // for (int d = 0; d < num_d; d += digits_per_it)
            {
                ...
                for (int d_ = d; d_ < d + ds; ++d_)
                {
                        ...
                        std::cout << "/** NTT special limbs */" << std::endl;
                    {
                        ...
                        if (size > 0)
                        {
                            NTT_ < false, algo, NTT_NONE >  << < dim3{cc.N / (blockDimFirst.x * M * 2), size},
                                blockDimFirst, bytesFirst, stream1.ptr()
                                >
                                >
                                >
                                (
        ...
        {
            {
                ...
                else
                {
                    ...
                    exec_old = map_exec.emplace(*level, cached_graph{
                                                    .first = graph, .second = nullptr, .buffC0 = c0.uid,
                                                    .buffC1 = c1.uid
                                                }).first;
                }
                ...
                if (!ok)
                {
                    ...
                    {
                        map_exec.emplace(*level, cached_graph{
                                             .first = exec_old->second.first, .second = exec, .buffC0 = c0.uid,
                                             .buffC1 = c1.uid
                                         });
...

        {
            ...
            if (!PEER_ACCESS && cc.GPUid.size() > 1)
            {
                if (MEMCPY_PEER)
                {
                    for (uint32_t j = 0; j < cc.GPUid.size(); ++j)
                    {
                        cc.digitStreamForMemcpyPeer[0].at(j).at(id).wait(s);
                        // to pass dependency up to the communication
                    }
                    CudaCheckErrorModNoSync;
                    openmp_synchronize();
                    ...
                    for (size_t i = 0; i < cc.splitSpecialMeta.size(); ++i)
                    {
                        ...
                        if (static_cast<uint32_t>(id) == i)
                        {
                            if (num_limbs > 0)
                            {
                                uint64_t* ptr = auxLimbs.bufferSPECIAL + (cc.splitSpecialMeta.at(i).at(0).id -
                                    SPECIALmeta.at(0).id) * cc.N;
                                ...
                                for (uint32_t j = 0; j < cc.GPUid.size(); ++j)
                                {
                                    if (j != i)
                                    {
                                        ...
                                        CudaCheckErrorModNoSync;
                                        cudaMemcpyPeerAsync(
                                            bufferSpecial_[j] + (cc.splitSpecialMeta.at(i).at(0).id - SPECIALmeta.at(0).
                                                id) * cc.N,
                                            cc.GPUid[j],
                                            ptr,
                                            cc.GPUid[i],
                ...
                else
                {
                    ...
                    for (size_t i = 0; i < cc.splitSpecialMeta.size(); ++i)
                    {
                        ...
                        if (num_limbs > 0)
                        {
                            uint64_t* ptr = auxLimbs.bufferSPECIAL + (cc.splitSpecialMeta.at(i).at(0).id - SPECIALmeta.
                                at(0).id) * cc.N;
                            NCCLCHECK(ncclBroadcast(ptr, ptr, cc.N * num_limbs, ncclUint64, (int)i, rank,
                                                    stream.ptr()));
                        }
                    }
                    NCCLCHECK(ncclGroupEnd());
        ...
        if (limbsize - skip0 > 0)
        {
            ...
            if (MODRAISE_WITH_P0)
            {
                if (SPECIALmeta.size() > 0 && SPECIALmeta.at(0).id == cc.L + 1)
                    broadcastLimb0_mgpu_<<<dim3{(uint32_t)cc.N / 128, (uint32_t)1}, 128, 0, s.ptr()>>>(
                        SPECIALlimbptr.data, SPECIAL(id, 0), limbptr.data);
            }
        }
        s.wait(stream);
    }
