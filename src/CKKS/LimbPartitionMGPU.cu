    ...
    {
        {
            ...
            for (int i = 0; i < size; i += batch) {
                ...
                NTT_<false, algo, mode><<<dim3{ NTT_grid_dim_X<M, false>(logN), num_limbs }, blockDimFirst, bytesFirst, stream.ptr()>>>(
                    getGlobals(), ptr.data, PARTITION(id, i), auxptr.data + i, nullptr, -1, *level);
                ...
                NTT_<true, algo, mode><<<dim3{ NTT_grid_dim_X<M, false>(logN), num_limbs }, blockDimSecond, bytesSecond, s.ptr()>>>(
                    getGlobals(), auxptr.data + i, PARTITION(id, i), limbptr.data + i, nullptr, -1, *level);
            }
        }
    }
    if (cc.GPUid.size() > 1) {
    ...
    for (int d = 0; d < num_d; d += digits_per_it) {
            ...
            std::cout << "/** Communicate */" << std::endl;
        {
            if (!PEER_ACCESS && cc.GPUid.size() > 1) {
                if (MEMCPY_PEER) {
                    for (size_t i = 0; i < cc.GPUid.size(); ++i) {
                        cc.digitStreamForMemcpyPeer.at(d).at(i).at(id).wait(s);
                        // ensure gather buffer is created before peer memcpy, we use s as the dependency will be older than on stream
                    }
                    if (MEMCPY_PEER && GRAPH_CAPTURE) {
                        *thread_stop[id] += 1;
