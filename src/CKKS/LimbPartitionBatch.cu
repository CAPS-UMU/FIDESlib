...
{
    void LimbPartition::addBatchManyToOne(std::vector<LimbPartition*>& parta,
                                          ...
                                          bool extb)
    {
        ...
        uint32_t num_parallel_parts =
            std::max(1u, std::min(
                         (uint32_t)(
                             (threads + block.x * grid.x * grid.y - 1) / (block.x * grid.x * std::max(1u, grid.y))),
                         (uint32_t)parta.size()));
        ...
        int size = its * split * partb.size() + split * partb.size() + extb * (its * split * partb.size() + split *
            partb.size());
            ...
            if (!exta && extb)
            {
                if (limbsize > 0)
                    sub_scale_p_reuse_b___<<<grid, block, 0, s.ptr()>>>(data_ptrs_d, data_ptrs_d + offset1,
                                                                        PARTITION(parta[0]->id, 0), n, its);
                if (slimbsize > 0)
                    copy_reuse_negative_b___<<<sgrid, block, 0, s.ptr()>>>(
                        data_ptrs_d + offset2, data_ptrs_d + offset3, SPECIAL(parta[0]->id, 0), n, its);
            }
        }
        partb[0]->s.wait(s);
    ...
    void LimbPartition::multPtBatchManyToOne(std::vector<LimbPartition*>& parta,
                                             const std::vector<LimbPartition*>& partb, int stride, double usage)
    {
        ContextData& cc = parta[0]->cc;
        int limbsize = parta[0]->getLimbSize(*parta[0]->level);
        int n = parta.size();
                                                 ...
                                                 double usage)
    {
        ...
        for (uint32_t i = 0; i < vector.size(); ++i)
        {
            ...
            {
                std::memcpy(
                    &data_ptrs[its * split * vector.size() + split * vector.size() * MAXP + (i * split + j) * MAXP],
                    vector_shoup[i].data(),
        ...
        if (limbsize > 0)
            mult_scalar_reuse_b___<<<grid, block, 0, s.ptr()>>>(
                data_ptrs_d,
                data_ptrs_d + its * split * vector.size(),
                data_ptrs_d + its * split * vector.size() + split * vector.size() * MAXP,
                PARTITION(parta[0]->id, 0),
                n,
                its);
        cudaFreeAsync(data_ptrs_d, s.ptr());
    }
                                            ...
                                            int slots)
    {
        ...
        uint32_t num_parallel_parts = std::max(1u,
                                               std::min(
                                                   (uint32_t)(
                                                       (threads + block.x * block.y * block.z * grid.x * grid.y - 1) / (
        ...
        for (int i = 0; i < num_LT; ++i)
        {
            for (int j = 0; j < stride; ++j)
            {
                ...
                {
                    data_ptrs[offset_out_c0 + i * stride * gStep + j * gStep + k] = out[2 * (i * stride * gStep + j *
                        gStep + k)]->limbptr.data;
                }
            }
            ...
            for (int j = 0; j < stride; ++j)
            {
                for (int k = 0; k < gStep; ++k)
                {
                    data_ptrs[offset_out_c1 + i * stride * gStep + j * gStep + k] = out[2 * (i * stride * gStep + j *
                        gStep + k) + 1]->limbptr.data;
                }
            }
            ...
            for (int j = 0; j < stride; ++j)
            {
                ...
                {
                    data_ptrs[offset_in_c0 + i * stride * bStep + j * bStep + k] = in[2 * (i * stride * bStep + j *
                        bStep + k)]->limbptr.data;
                }
            }
            ...
            for (int j = 0; j < stride; ++j)
            {
                for (int k = 0; k < bStep; ++k)
                {
                    data_ptrs[offset_in_c1 + i * stride * bStep + j * bStep + k] = in[2 * (i * stride * bStep + j *
                        bStep + k) + 1]->limbptr.data;
                }
            }
                ...
                for (int k = 0; k < bStep; ++k)
                {
                    data_ptrs[offset_pt + i * gStep * bStep + j * bStep + k] =
                        pt[i * gStep * bStep + j * bStep + k]
                            ? pt[i * gStep * bStep + j * bStep + k]->limbptr.data
                            : nullptr;
                }
            }
            ...
            if (ext)
            {
                for (int j = 0; j < stride; ++j)
                {
                    for (int k = 0; k < gStep; ++k)
                    {
                        data_ptrs[soffset_out_c0 + i * stride * gStep + j * gStep + k] = out[2 * (i * stride * gStep + j
                                * gStep + k)]->SPECIALlimbptr.data +
                            special_start;
                    }
                }
                ...
                for (int j = 0; j < stride; ++j)
                {
                    for (int k = 0; k < gStep; ++k)
                    {
                        data_ptrs[soffset_out_c1 + i * stride * gStep + j * gStep + k] = out[2 * (i * stride * gStep + j
                                * gStep + k) + 1]->SPECIALlimbptr.data +
                            special_start;
                    }
                }
                ...
                for (int j = 0; j < stride; ++j)
                {
                    for (int k = 0; k < bStep; ++k)
                    {
                        data_ptrs[soffset_in_c0 + i * stride * bStep + j * bStep + k] = in[2 * (i * stride * bStep + j *
                                bStep + k)]->SPECIALlimbptr.data +
                            special_start;
                    }
                }
                ...
                for (int j = 0; j < stride; ++j)
                {
                    for (int k = 0; k < bStep; ++k)
                    {
                        data_ptrs[soffset_in_c1 + i * stride * bStep + j * bStep + k] = in[2 * (i * stride * bStep + j *
                                bStep + k) + 1]->SPECIALlimbptr.data +
                            special_start;
                    }
                }
                                                ...
                                                bool c0_modup)
    {
        ...
        int size = 2 * out.size() + in.size() + (num_d + 1) * in.size() / 2 + (num_d + 1) * (ksk_a.size() + ksk_b.
            size()) + (indexes.size() + 1) / 2;
        int offset_out_c0 = 0;
        ...
        for (uint32_t j = 0; j < ksk_a.size(); ++j)
        {
            for (uint32_t k = 0; k < num_d; ++k)
            {
                ...
                data_ptrs[offset_ksk + (num_d + 1) * (2 * j + 1) + k] = ksk_b[j]
                                                                            ? ksk_b[j]->DIGITlimbptr[k].data
                                                                            : nullptr;
            }
