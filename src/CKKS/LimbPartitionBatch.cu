...
void LimbPartition::LTdotProductPtBatch(std::vector<LimbPartition*>& out,
    ...
    // The launch is sized `grid.y = limbsize` from out[0]'s level, and the kernel indexes every
    // operand's pointer table by `limbsize`; a partition holding fewer limbs hands it a pointer past
    // the end. Check on the host. `nullptr` pt entries are fine: they encode a diagonal with no
    // plaintext, which the kernel skips.
