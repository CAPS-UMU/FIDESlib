...
__global__ void mult1AddMult23Add4_(const __grid_constant__ int primeid_init,
                                    void** l,
                                    void** l1,
                                    void** l2,
                                    void** l3,
...
__global__ void multnomoddownend_(const __grid_constant__ int primeid_init,
                                  void** c1,
                                  void** c0,
                                  void** bc0,
                                  void** bc1,
...
template <typename T> __device__ __forceinline__ void addMult__(T* l, const T* l1, const T* l2, const int primeid, const int elem_l2) {
	const int idx       = threadIdx.x + blockDim.x * blockIdx.x;
	constexpr ALGO algo = ALGO_BARRETT;
...
template <typename T> __global__ void addMult_(T* l,
                                               const T* l1,
                                               const T* l2,
                                               const __grid_constant__ int primeid,
                                               const __grid_constant__ int elem_l2) {
	addMult__<T>(l, l1, l2, primeid, elem_l2);
}

__global__ void addMult_(void** l,
                         void** l1,
                         void** l2,
	...
	if (ISU64(primeid)) {
		addMult__<uint64_t>((uint64_t*)l[blockIdx.y],
		                    (uint64_t*)l1[blockIdx.y],
		                    (uint64_t*)l2[blockIdx.y],
		                    primeid,
		                    elem_l2);
	} else {
		addMult__<uint32_t>((uint32_t*)l[blockIdx.y],
		                    (uint32_t*)l1[blockIdx.y],
		                    (uint32_t*)l2[blockIdx.y],
		                    primeid,
		                    elem_l2);
	}
}

__global__ void Mult_(void** l,
                      void** l1,
                      void** l2,
                      const __grid_constant__ int primeid_init,
                      const __grid_constant__ int elem_l2) {
	const int primeid   = C_.primeid_flattened[primeid_init + blockIdx.y];
	const int idx       = threadIdx.x + blockDim.x * blockIdx.x;
	const int logstride = __ffs(blockDim.x * gridDim.x / elem_l2) - 1;
	const int l2idx     = idx >> logstride;
	//    if (idx == 0)
	//        printf("%d %d\n", primeid_init + blockIdx.y, primeid);
	if (ISU64(primeid)) {
		((uint64_t*)l[blockIdx.y])[idx] = modmult<ALGO_BARRETT>(((uint64_t*)l1[blockIdx.y])[idx],
		                                                        ((uint64_t*)l2[blockIdx.y])[l2idx],
		                                                        primeid);
	} else {
		((uint32_t*)l[blockIdx.y])[idx] = modmult<ALGO_BARRETT>(((uint32_t*)l1[blockIdx.y])[idx],
		                                                        ((uint32_t*)l2[blockIdx.y])[l2idx],
		                                                        primeid);
	}
}
...
__global__ void binomial_square_fold_(void** c0_res,
                                      void** c2_key_switched_0,
                                      void** c1,
                                      void** c2_key_switched_1,
                                      const __grid_constant__ int primeid_init) {
...
template <ALGO algo> __global__ void Scalar_mult_(void** a,
                                                  const uint64_t* b,
                                                  const __grid_constant__ int primeid_init,
	...
	if (ISU64(primeid)) {
		((uint64_t*)a[blockIdx.y])[idx] = modmult<algo>(((uint64_t*)a[blockIdx.y])[idx],
		                                                b[primeid],
		                                                primeid,
		                                                shoup_mu ? shoup_mu[primeid] : 0);
	} else {
...
__global__ void eval_linear_w_sum_(const __grid_constant__ int n,
                                   void** a,
                                   void*** bs,
                                   uint64_t* w,
                                   const __grid_constant__ int primeid_init) {
...
__global__ void eval_linear_w_sum_with_bias_(const __grid_constant__ int n,
                                             void** a,
                                             void*** bs,
                                             uint64_t* w,
                                             const __grid_constant__ int primeid_init) {
...
__global__ void fusedDotKSK_2_(void** out1,
	...
	*/
	...
	for (int i = 0; i < num_d; ++i) {
		...
		__uint128_t add1 =
			(__uint128_t)in * ((uint64_t*)digits[C_.dnum + i + decomp * 3 * C_.dnum][decomp ? pos_dec : pos])[idx];
		__uint128_t add2 = (__uint128_t)in * ((uint64_t*)digits[2 * C_.dnum + i + decomp * 3 * C_.dnum][
			decomp ? pos_dec : pos])[idx];
	...
	if (primeid < C_.L) {
		((uint64_t*)out1[pos_dec])[idx] = modreduce<ALGO_SHOUP>(aux1, primeid);
		((uint64_t*)out2[pos_dec])[idx] = modreduce<ALGO_SHOUP>(aux2, primeid);
	} else {
		((uint64_t*)sout1[primeid - C_.L])[idx] = modreduce<ALGO_SHOUP>(aux1, primeid);
		((uint64_t*)sout2[primeid - C_.L])[idx] = modreduce<ALGO_SHOUP>(aux2, primeid);
	}
}
	...
	for (int j = 0; j < n; ++j) {
		...
		for (int i = 0; i < num_d; ++i) {
			...
			uint64_t add1 = modmult<ALGO_BARRETT>(in1[i],
			                                      ((uint64_t*)digits[offset + C_.dnum + i + decomp * 3 * C_.dnum][
				                                      decomp ? pos_dec : pos])[idx],
			                                      primeid);
			...
			if (PRINT && idx == 0 && blky == 6)
				printf("kska %d : %lu\n",
				       i,
				       ((uint64_t*)digits[offset + C_.dnum + i + decomp * 3 * C_.dnum][decomp ? pos_dec : pos])[
					       idx]);

			if (PRINT && idx == 0 && blky == 6)
				printf("add1 %d : %lu\n", i, add1);
			uint64_t add2 = modmult<ALGO_BARRETT>(in1[i],
			                                      ((uint64_t*)digits[offset + 2 * C_.dnum + i + decomp * 3 * C_.
				                                      dnum][decomp ? pos_dec : pos])[idx],
			                                      primeid);

			if (PRINT && idx == 0 && blky == 6)
				printf("kskb %d : %lu\n",
				       i,
				       ((uint64_t*)digits[offset + 2 * C_.dnum + i + decomp * 3 * C_.dnum][decomp ? pos_dec : pos])[
					       idx]);
			if (PRINT && idx == 0 && blky == 6)
				printf("add2 %d : %lu\n", i, add2);
...
*/
	...
	for (uint32_t i = threadIdx.y; i < num_d; i += blockDim.y) {
		...
		in1[i * stride_in] = ((uint64_t*)(decomp ? c1[threadIdx.z] : din1[num_d * threadIdx.z + i])[
			decomp ? pos_dec : pos])[idx];
	...
	for (int j = 0; j < n; ++j) {
		...
		if (digits[(2 * j + threadIdx.y) * (num_d + 1)]) {
			...
			if (threadIdx.x == 0) {
				...
				uint64_t** ksk_from = (uint64_t**)digits[(2 * j + threadIdx.y) * (num_d + 1) + (
					decomp ? num_d : i)];
			...
			if (PRINT && idx == 0 && blky == 6)
				printf("%d : %d %d %d %lu\n",
				       ...
				       indexes[j],
				       C_.logN,
				       automorph_slot(C_.logN, indexes[j], idx),
				       aux);
...
__global__ void dotProductPt_(void** c0,
	...
	const int idx       = threadIdx.x + blockIdx.x * blockDim.x;
	const int logstride = __ffs(blockDim.x * gridDim.x / elem_data) - 1;
	const int dataidx   = idx >> logstride;
	const int primeid   = C_.primeid_flattened[primeidInit + blockIdx.y];
	constexpr ALGO algo = ALGO_BARRETT;

	uint64_t out0, out1;
	uint64_t in = ((uint64_t*)data[n * 2][ptroffset + blockIdx.y])[dataidx];
	if (PRINT && idx == 0 && blockIdx.y == 0)
		printf("LT:, b: %d, g:, in pt: %lu \n", 0, in);
	out0 = modmult<algo>(in, ((uint64_t*)data[0][ptroffset + blockIdx.y])[dataidx], primeid);

	if (PRINT && idx == 0 && blockIdx.y == 0)
		printf("LT: %d, b: %d, in c0: %lu \n", -1, 0, ((uint64_t*)data[0][ptroffset + blockIdx.y])[dataidx]);

	out1 = modmult<algo>(in, ((uint64_t*)data[n][ptroffset + blockIdx.y])[dataidx], primeid);

	if (PRINT && idx == 0 && blockIdx.y == 0)
		printf("LT: %d, b: %d, in c1: %lu \n", -1, 0, ((uint64_t*)data[n][ptroffset + blockIdx.y])[dataidx]);

	for (int i = 1; i < n; ++i) {
		in = ((uint64_t*)data[n * 2 + i][ptroffset + blockIdx.y])[dataidx];

		if (PRINT && idx == 0 && blockIdx.y == 0)
			printf("LT:, b: %d, g:, in pt: %lu \n", i, in);

		uint64_t aux0 = modmult<algo>(in, ((uint64_t*)data[i][ptroffset + blockIdx.y])[dataidx], primeid);

		if (PRINT && idx == 0 && blockIdx.y == 0)
			printf("LT: %d, b: %d, in c0: %lu \n", -1, i, ((uint64_t*)data[i][ptroffset + blockIdx.y])[dataidx]);

		uint64_t aux1 = modmult<algo>(in, ((uint64_t*)data[n + i][ptroffset + blockIdx.y])[dataidx], primeid);

		if (PRINT && idx == 0 && blockIdx.y == 0)
			printf("LT: %d, b: %d, in c1: %lu \n",
			       -1,
			       i,
			       ((uint64_t*)data[n + i][ptroffset + blockIdx.y])[dataidx]);
                ...
                if constexpr (0) {
                    ...
                    if (threadIdx.z == 0) {
                        ...
                        ((uint64_t*)outputs[k * batch * gStep + i * gStep + j][blockIdx.y])[idx] =
                            modreduce<ALGO_SHOUP>(res, primeid);
                        if (PRINT && idx == 0 && blockIdx.y == 0)
                    ...
                    if (threadIdx.z == 0) {
                        ...
                        ((uint64_t*)outputs[k * batch * gStep + i * gStep + j][blockIdx.y])[idx] =
                            modreduce<ALGO_SHOUP>(acc_this_thread[0], primeid);
                        if (PRINT && idx == 0 && blockIdx.y == 0)
                            printf("LT: %d, g: %d, res: %lu \n", k, j, acc_this_thread[0]);
                    }
                }
...
dotProductLtBatchedPt2___(void*** c0_out,
	...
	const int idx       = threadIdx.x + threadIdx.z * blockDim.x + blockIdx.x * blockDim.x * blockDim.z;
	const int logstride = __ffs(blockDim.z * blockDim.x * gridDim.x / elem_pts) - 1;
	const int ptsidx    = idx >> logstride;
	...
	for (int k = blockIdx.z; k < n; k += gridDim.z) {
		for (int i = 0; i < bStep; ++i) {
			...
			for (int j = 0; j < gStep; ++j) {
				...
				if (pt_partition != nullptr) {
					// pt[0] = ((uint64_t*)pt_partition[blockIdx.y])[idx];
					mult = modmult<ALGO_BARRETT>(in, ((uint64_t*)pt_partition[blockIdx.y])[ptsidx], primeid);
				}
				if (i == 0)
	...
	for (int k = blockIdx.z; k < n; k += gridDim.z) {
		for (int i = 0; i < bStep; ++i) {
			...
			for (int j = 0; j < gStep; ++j) {
				...
				if (pt_partition != nullptr) {
					// pt[0] = ((uint64_t*)pt_partition[blockIdx.y])[idx];
					mult = (__uint128_t)in * (__uint128_t)((uint64_t*)pt_partition[blockIdx.y])[ptsidx];
				}
				if (i == 0)
					acc_this_thread[j * in_stride] = mult;
				else
				...
				if (i == bStep - 1) {
					uint64_t res                                         = modreduce<ALGO_SHOUP>(acc_this_thread[j * in_stride], primeid);
...
dotProductLtBatchedPt___(void*** c0_out,
                         ...
                         const __grid_constant__ int elem_pts) {
	int idx             = threadIdx.x + blockIdx.x * blockDim.x;
	const int logstride = __ffs(blockDim.x * gridDim.x / elem_pts) - 1;
	const int ptsidx    = idx >> logstride;
	...
	for (int k = blockIdx.z; k < n; k += gridDim.z) {
		...
		for (int j = 0; j < gStep; ++j) {
			...
			if (pt_partition != nullptr) {
				if (im_c0) {
					pt[0] = ((uint64_t*)pt_partition[blockIdx.y])[ptsidx];
					if (PRINT && idx == 0 && blockIdx.y == 0)
						printf("LT: %d, b: %d, g: %d, in pt: %lu %lu %p %p\n",
						       k,
						       b_idx,
			...
			for (int i_0 = 0; i_0 < batch; i_0 += 2) {
				if (pt_partition != nullptr) {
					...
					if (i_0 + 1 < batch)
						acc_this_thread[in_stride] = modmult<ALGO_BARRETT>(
							in[in_stride * (i_0 + 1)],
							pt[0],
							primeid);
				} else {
...

				{
					...
					if (r > 0) {
						__syncthreads();
						if (threadIdx.z + r < b) {
							acc_this_thread[0] = modadd(acc_this_thread[0],
							                            acc_this_thread[0 + r * blockDim.x * blockDim.y],
							                            primeid);
						}
						if (threadIdx.z >= r) {
							acc_this_thread[in_stride] = modadd(acc_this_thread[in_stride],
							                                    acc_this_thread[in_stride - r * blockDim.x *
								                                    blockDim.y],
							                                    primeid);
						}
					}
					...
					if (threadIdx.z == 0) {
						((uint64_t*)outputs[k * batch * gStep + i_0 * gStep + j][blockIdx.y])[idx] = acc_this_thread
							[0];
						if (PRINT && idx == 0 && blockIdx.y == 0)
							printf("LT: %d, g: %d, res: %lu \n", k, j, acc_this_thread[0]);
					}
...
__global__ void addScaleB_(void** a, void** b, void** c, const int primeid_init, const __grid_constant__ int elem_c) {
	...
	if (ISU64(primeid)) {
		uint64_t in = modmult<ALGO_SHOUP>(((uint64_t*)b[blockIdx.y])[idx],
		                                  C_.P[primeid],
		                                  primeid,
		                                  C_.P_shoup[primeid]);
		((uint64_t*)a[blockIdx.y])[idx] = modadd(in, ((uint64_t*)c[blockIdx.y])[cidx], primeid);
	} else {
	}
}
...
__global__ void addScaleC_(void** a, void** b, void** c, const int primeid_init, const __grid_constant__ int elem_c) {
	...
	if (ISU64(primeid)) {
		uint64_t in = modmult<ALGO_SHOUP>(((uint64_t*)c[blockIdx.y])[cidx],
		                                  C_.P[primeid],
		                                  primeid,
		                                  C_.P_shoup[primeid]);
		((uint64_t*)a[blockIdx.y])[idx] = modadd(in, ((uint64_t*)b[blockIdx.y])[idx], primeid);
	} else {
	}
}
...
__global__ void scaleByP_(void** a, const int primeid_init) {
	...
	const int idx     = blockIdx.x * blockDim.x + threadIdx.x;
	if (ISU64(primeid)) {
		uint64_t in = modmult<ALGO_SHOUP>(((uint64_t*)a[blockIdx.y])[idx],
		                                  C_.P[primeid],
		                                  primeid,
		                                  C_.P_shoup[primeid]);
		((uint64_t*)a[blockIdx.y])[idx] = in;
	...
	if (ISU64(primeid)) {
		for (int i = blockIdx.z; i < n / its; i += gridDim.z) {
			...
			for (int j = 0; j < its && a[i * its + j] != nullptr; ++j) {
				((uint64_t*)(a[i * its + j][blockIdx.y]))[idx] = modadd(
		...
		for (int i = blockIdx.z; i < n / its; i += gridDim.z) {
			...
			for (int j = 0; j < its && a[i * its + j] != nullptr; ++j) {
				((uint32_t*)(a[i * its + j][blockIdx.y]))[idx] = modadd(
...
__global__ void sub_reuse_scale_p_b___(void*** a, void*** b, const int primeid_init, const int n, const int its) {
	...
	if (ISU64(primeid)) {
		for (int i = blockIdx.z; i < n / its; i += gridDim.z) {
			...
			load          = modmult<ALGO_SHOUP>(load, C_.P[primeid], primeid, C_.P_shoup[primeid]);
			for (int j = 0; j < its && a[i * its + j] != nullptr; ++j) {
				((uint64_t*)(a[i * its + j][blockIdx.y]))[idx] = modsub(
					((uint64_t*)(a[i * its + j][blockIdx.y]))[idx],
					load,
					primeid);
			}
		...
		for (int i = blockIdx.z; i < n / its; i += gridDim.z) {
			...
			load          = modmult<ALGO_SHOUP>(load, (uint32_t)C_.P[primeid], primeid, (uint32_t)C_.P_shoup[primeid]);
			for (int j = 0; j < its && a[i * its + j] != nullptr; ++j) {
				((uint32_t*)(a[i * its + j][blockIdx.y]))[idx] = modsub(
					((uint32_t*)(a[i * its + j][blockIdx.y]))[idx],
					load,
					primeid);
			}
...
__global__ void add_scale_p_reuse_b___(void*** a, void*** b, const int primeid_init, const int n, const int its) {
	...
	if (ISU64(primeid)) {
		for (int i = blockIdx.z; i < n / its; i += gridDim.z) {
			uint64_t load = ((uint64_t*)(b[i][blockIdx.y]))[idx];
			for (int j = 0; j < its && a[i * its + j] != nullptr; ++j) {
				uint64_t aux = modmult<ALGO_SHOUP>(((uint64_t*)(a[i * its + j][blockIdx.y]))[idx],
				                                   C_.P[primeid],
				                                   primeid,
				                                   C_.P_shoup[primeid]);
...
__global__ void mult_scalar_reuse_b___(void*** a,
                                       void*** b,
                                       void*** b_shoup,
	...
	if (ISU64(primeid)) {
		for (int i = blockIdx.z; i < n / its; i += gridDim.z) {
			...
			for (int j = 0; j < its && a[i * its + j] != nullptr; ++j) {
				((uint64_t*)(a[i * its + j][blockIdx.y]))[idx] = modmult<ALGO_SHOUP>(
					((uint64_t*)(a[i * its + j][blockIdx.y]))[idx],
					load,
					primeid,
					load_shoup);
		...
		for (int i = blockIdx.z; i < n / its; i += gridDim.z) {
			...
			for (int j = 0; j < its && a[i * its + j] != nullptr; ++j) {
				((uint32_t*)(a[i * its + j][blockIdx.y]))[idx] = modmult<ALGO_SHOUP>(
					((uint32_t*)(a[i * its + j][blockIdx.y]))[idx],
					load,
					primeid,
					load_shoup);
...
__global__ void mult_reuse_b___(void*** a, void*** b, const int primeid_init, const int n, const int its) {
	...
	if (ISU64(primeid)) {
		for (int i = blockIdx.z; i < n / its; i += gridDim.z) {
			uint64_t load = ((uint64_t*)(b[i][blockIdx.y]))[idx];
			for (int j = 0; j < its && a[i * its + j] != nullptr; ++j) {
				((uint64_t*)(a[i * its + j][blockIdx.y]))[idx] = modmult<ALGO_BARRETT>(
					((uint64_t*)(a[i * its + j][blockIdx.y]))[idx],
					load,
					primeid);
			}
		...
		for (int i = blockIdx.z; i < n / its; i += gridDim.z) {
			uint32_t load = ((uint32_t*)(b[i][blockIdx.y]))[idx];
			for (int j = 0; j < its && a[i * its + j] != nullptr; ++j) {
				((uint32_t*)(a[i * its + j][blockIdx.y]))[idx] = modmult<ALGO_BARRETT>(
					((uint32_t*)(a[i * its + j][blockIdx.y]))[idx],
					load,
					primeid);
			}
...
__global__ void binomialMult_(const __grid_constant__ int primeid_init,
                              void** c0,
                              void** c1,
                              void** c2,
                              void** d0,
	...
	if (ISU64(primeid)) {
		...
		T aux1 = modadd(modmult<ALGO_BARRETT>(c0in, d1in, primeid),
		                modmult<ALGO_BARRETT>(c1in, d0in, primeid),
		                primeid);
...
__global__ void binomialMultExtend_(const __grid_constant__ int primeid_init,
                                    void** c0,
                                    void** c1,
                                    void** c2,
                                    void** d0,
...
__global__ void binomialSquareExtend_(const __grid_constant__ int primeid_init, void** c0, void** c1, void** c2) {
	...
	if (ISU64(primeid)) {
		...
		T aux1                      = modmult<ALGO_BARRETT>(c0in, c1in, primeid);
		((T*)(c1[blockIdx.y]))[idx] = modmult<ALGO_SHOUP>(modadd(aux1, aux1, primeid),
		                                                  C_.P[primeid],
		                                                  primeid,
		                                                  C_.P_shoup[primeid]);
	...
	if (ISU64(primeid)) {
		...
		for (int i = blockIdx.z; i < n; i += gridDim.z) {
			...
			for (int j = 0; j < its && c0[i * its + j] != nullptr; ++j) {
				...
				if (ext_in) {
					d0in = modmult<ALGO_SHOUP>(d0in,
					                           (uint32_t)C_.P_inv[primeid],
					                           primeid,
					                           (uint32_t)C_.P_inv_shoup[primeid]);
					d1in = modmult<ALGO_SHOUP>(d1in,
					                           (uint32_t)C_.P_inv[primeid],
					                           primeid,
					                           (uint32_t)C_.P_inv_shoup[primeid]);
				}
		...
		for (int i = blockIdx.z; i < n; i += gridDim.z) {
			...
			for (int j = 0; j < its && c0[i * its + j] != nullptr; ++j) {
...

				{
					...
					c0in    = modadd(modmult<ALGO_BARRETT>(c0in, pt1in, primeid),
					                 modmult<ALGO_BARRETT>(c0in, pt2in, primeid),
					                 primeid);
					c1in = modadd(modmult<ALGO_BARRETT>(c1in, pt1in, primeid),
					              modmult<ALGO_BARRETT>(c1in, pt2in, primeid),
					              primeid);
				}
...
template __global__ void FIDESlib::CKKS::addMult_<uint64_t>(uint64_t* l,
                                                            const uint64_t* l1,
                                                            const uint64_t* l2,
                                                            const __grid_constant__ int primeid,
                                                            const __grid_constant__ int elem_l2);

template __global__ void FIDESlib::CKKS::addMult_<uint32_t>(uint32_t* l,
                                                            const uint32_t* l1,
                                                            const uint32_t* l2,
                                                            const __grid_constant__ int primeid,
                                                            const __grid_constant__ int elem_l2);
