...
__global__ void addMult_(void** l, void** l1, void** l2, const __grid_constant__ int primeid_init, const __grid_constant__ int elem_l2) {
    ...
    if (ISU64(primeid)) {
        addMult__<uint64_t>((uint64_t*)l[blockIdx.y], (uint64_t*)l1[blockIdx.y], (uint64_t*)l2[blockIdx.y], primeid, elem_l2);
    } else {
        addMult__<uint32_t>((uint32_t*)l[blockIdx.y], (uint32_t*)l1[blockIdx.y], (uint32_t*)l2[blockIdx.y], primeid, elem_l2);
    }
}
...
__global__ void Mult_(void** l, void** l1, void** l2, const __grid_constant__ int primeid_init, const __grid_constant__ int elem_l2) {
    ...
    //        printf("%d %d\n", primeid_init + blockIdx.y, primeid);
    if (ISU64(primeid)) {
        ((uint64_t*)l[blockIdx.y])[idx] = modmult<ALGO_BARRETT>(((uint64_t*)l1[blockIdx.y])[idx], ((uint64_t*)l2[blockIdx.y])[l2idx], primeid);
    } else {
        ((uint32_t*)l[blockIdx.y])[idx] = modmult<ALGO_BARRETT>(((uint32_t*)l1[blockIdx.y])[idx], ((uint32_t*)l2[blockIdx.y])[l2idx], primeid);
    }
}
...
__global__ void fusedDotKSK_2_(void** out1, void** sout1, void** out2, void** sout2, void*** digits, int num_d, int id, int num_special, int init) {
    ...
    */
    ...
    for (int i = 0; i < num_d; ++i) {
        ...
        __uint128_t add1 = (__uint128_t)in * ((uint64_t*)digits[C_.dnum + i + decomp * 3 * C_.dnum][decomp ? pos_dec : pos])[idx];
        __uint128_t add2 = (__uint128_t)in * ((uint64_t*)digits[2 * C_.dnum + i + decomp * 3 * C_.dnum][decomp ? pos_dec : pos])[idx];
...
__global__ void dotProductPt_(void** c0, void** c1, void*** data, const size_t ptroffset, const int primeidInit, const int n, const __grid_constant__ int elem_data) {
    const int idx = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    for (int i = 1; i < n; ++i) {
        ...
        if (PRINT && idx == 0 && blockIdx.y == 0)
            printf("LT: %d, b: %d, in c1: %lu \n", -1, i, ((uint64_t*)data[n + i][ptroffset + blockIdx.y])[dataidx]);
...
__global__ void
dotProductLtBatchedPt2___(void*** c0_out, void*** c1_out, void*** c0_in, void*** c1_in, void*** pts, const int bStep, const int gStep, const int primeidInit, const int n, const __grid_constant__ int elem_pts) {
...
__global__ void
dotProductLtBatchedPt3___(void*** c0_out, void*** c1_out, void*** c0_in, void*** c1_in, void*** pts, const int bStep, const int gStep, const int primeidInit, const int n, const __grid_constant__ int elem_pts) {
    const int idx = threadIdx.x + threadIdx.z * blockDim.x + blockIdx.x * blockDim.x * blockDim.z;
    const int logstride = __ffs(blockDim.z * blockDim.x * gridDim.x / elem_pts) - 1;
    const int ptsidx = idx >> logstride;
...
__global__ void
dotProductLtBatchedPt___(void*** c0_out, void*** c1_out, void*** c0_in, void*** c1_in, void*** pts, const int batch, const int gStep, const int primeidInit, const int n, const __grid_constant__ int elem_pts) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void addScaleC_(void** a, void** b, void** c, const int primeid_init, const __grid_constant__ int elem_c) {
    ...
    if (ISU64(primeid)) {
        uint64_t in = modmult<ALGO_SHOUP>(((uint64_t*)c[blockIdx.y])[cidx], C_.P[primeid], primeid, C_.P_shoup[primeid]);
...
FIDESlib::CKKS::addMult_<uint64_t>(uint64_t* l, const uint64_t* l1, const uint64_t* l2, const __grid_constant__ int primeid, const __grid_constant__ int elem_l2);

template __global__ void
FIDESlib::CKKS::addMult_<uint32_t>(uint32_t* l, const uint32_t* l1, const uint32_t* l2, const __grid_constant__ int primeid, const __grid_constant__ int elem_l2);
