        ...
        if constexpr (algo != 3) {
            if (ISU64(primeid)) {
                ...
                buff[tid + blockDim.x * i_] = modmult<algo>(
                    /*((uint64_t*)(a[pos]))[idx]*/ val, G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(d, n_d_n - 1, primeid)], primeid);
            } else {
...
__device__ __forceinline__
    ...
    void
    named_bar_sync(int barrier_id, int thread_count) {
...
__device__ __forceinline__
    ...
    void
    named_bar_arrive(int barrier_id, int thread_count) {
