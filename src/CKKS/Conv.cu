...
namespace FIDESlib::CKKS
{
    ...
    template <ALGO algo>
    __global__ void
    ModDown2(void** __restrict__ a, const __grid_constant__int n, void** __restrict__ b,
             const __grid_constant__int primeid_init, const Global::Globals* Globals)
    {
        const int idx = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        {
            int primeid = i + C_.L;
            if constexpr (USING_CONSTANTS_TABLE)
            {
                // using constants table
                constexpr ALGO algo_ = algo == ALGO_SHOUP ? ALGO_BARRETT : algo;
                if (ISU64(primeid))
                {
                    buff[tid + blockDim.x * i] = modmult < algo_ > (((uint64_t*)(b[i]))[idx], TABLE64(C_.L, C_.L + i),
                        C_.L + i);
                }
                else
                {
                    buff[tid + blockDim.x * i] = modmult < algo_ > (((uint32_t*)b[i])[idx],
                        (uint32_t)TABLE32(C_.L, C_.L + i), C_.L + i);
                }
            }
            else
            {
                if constexpr (algo != 3)
                {
                    if (ISU64(primeid))
                    {
                        buff[tid + blockDim.x * i] = modmult < algo > (((uint64_t*)(b[i]))[idx],
                            G_->ModDown_pre_scale[primeid], primeid);
                    }
                    else
                    {
                        buff[tid + blockDim.x * i] = modmult < algo > ((uint64_t)((uint32_t*)b[i])[idx],
                            G_->ModDown_pre_scale[primeid], primeid);
                    }
                }
                else
    ...
    */
        ...
        for (int j = threadIdx.y; j < n; j += blockDim.y)
        {
            ...
            if constexpr (1)
            {
                ...
                {
                    res = res + (__uint128_t)buff[i * blockDim.x + tid] * G_->ModDown_matrix[
                        MODDOWN_MATRIX(i, primeid)];
                }

                // TODO use better reduction
                if (!ISU64(primeid))
                {
                    ((uint32_t*)a[j])[idx] = (uint32_t)modreduce<ALGO_SHOUP>(res, primeid);
                }
                else
                {
                    ((uint64_t*)a[j])[idx] = (uint64_t)modreduce<ALGO_SHOUP>(res, primeid);
                }
            }
            else
            {
                ...
                for (int i = 0; i < C_.K; ++i)
                {
                    if constexpr (USING_CONSTANTS_TABLE)
                    {
                        // using constants table
                        constexpr ALGO algo_ = algo == ALGO_SHOUP ? ALGO_BARRETT : algo;
                        uint64_t aux = modmult < algo_ > (buff[i * blockDim.x + tid], (uint64_t)TABLE64(C_.L + i, j),
                            j);
                    ...
                    else
                    {
                        if constexpr (algo != 3)
                        {
                            uint64_t aux = modmult < algo > (buff[i * blockDim.x + tid],
                                G_->ModDown_matrix[MODDOWN_MATRIX(i, j)], j);
                        ...
                        {
                            uint64_t aux =
                                modmult < algo > (buff[i * blockDim.x + tid], G_->ModDown_matrix[MODDOWN_MATRIX(i, j)],
                                    j,
...
#undef YY
    ...
    template <ALGO algo>
    __global__ void
    ModDown3(void** __restrict__ a, const __grid_constant__int n, void** __restrict__ b,
             const __grid_constant__int primeid_init, const Global::Globals* Globals)
    {
        const int idx = blockIdx.x * blockDim.x + threadIdx.x;
        const int tid = threadIdx.x;
                ...
                if (i_ + STAGES < n_half && pos + STAGES < C_.K)
                {
                    ...
                    if (ISU64(primeid))
                    {
                        if (pos + STAGES < C_.K)
                        {
                            ...
                            cuda::memcpy_async(
                                reinterpret_cast<ulonglong2*>(buff) + (blockDim.x * (pos + STAGES)) + tid,
                                reinterpret_cast<ulonglong2*>(peer_ptr) + (blockIdx.x * blockDim.x) + tid,
                                sizeof(ulonglong2),
                                pipe);
                        }
                    ...
                    if constexpr (algo != 3)
                    {
                        ...
                        else
                        {
                            buff[tid + blockDim.x * pos] = modmult < algo > (
                                buff[tid + blockDim.x * pos], G_->ModDown_pre_scale[primeid], primeid);
                        }
                    }
                    else
                    {
                        ...
                        else
                        {
                            buff[tid + blockDim.x * pos] =
                                modmult < algo > (buff[tid + blockDim.x * pos], G_->ModDown_pre_scale[primeid], primeid,
                                    G_->ModDown_pre_scale_shoup[primeid]);
                        }
                    }
                }
        ...
        for (int j_ = threadIdx.y; j_ < n; j_ += blockDim.y)
        {
            ...
            if (!ISU64(primeid))
            {
                ((uint32_t*)a[j_])[idx] = (uint32_t)modreduce<ALGO_SHOUP>(resx, primeid);
            }
            else
            {
                ulonglong2 res = {modreduce<ALGO_SHOUP>(resx, primeid), modreduce<ALGO_SHOUP>(resy, primeid)};
...
#undef YY
    ...
    template <ALGO algo>
    __global__ void
    DecompAndModUpConv(void** __restrict__ a, const int __grid_constant__ n, void** __restrict__ b,
            ...
            if constexpr (algo != 3)
            {
                if (ISU64(primeid))
                {
                    ...
                    buff[tid + blockDim.x * i_] =
                        modmult < algo > (
                            /*((uint64_t*)(a[pos]))[idx]*/ val, G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(
                                d, n_d_n - 1, primeid)], primeid);
                }
                else
                {
                    buff[tid + blockDim.x * i_] =
            ...
            else
            {
                ...
                else
                {
                    buff[tid + blockDim.x * i_] = modmult < algo > ((uint64_t)((uint32_t*)a[pos])[idx],
                        G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(
                            d, n_d_n - 1, primeid)],
                        primeid,
                        G_->DecompAndModUp_pre_scale_shoup[MODUPIDX_SCALE(
                            d, n_d_n - 1, primeid)]);
                }
            }
            /*
            ...
            if (primeid_j < n || primeid_j >= C_.L)
            {
                if constexpr (1)
                {
                    ...
                    for (int i_ = 0; i_ < n_d_n; ++i_)
                    {
                        ...
                        assert(MODUPIDX_MATRIX(n - 1, d, i_, primeid_j) < 64 * 64 * 64 * 8);
                        res = res + (__uint128_t)buff[i_ * blockDim.x + tid] * G_->DecompAndModUp_matrix[
                            MODUPIDX_MATRIX(n - 1, d, primeid /*i_*/, primeid_j)];

                        if (0)
                        {
                            uint64_t aux = G_->DecompAndModUp_matrix[MODUPIDX_MATRIX(
                                n - 1, d, primeid /*i_*/, primeid_j)];
                    ...
                    if (!ISU64(primeid_j))
                    {
                        ((uint32_t*)b[j_])[idx] = (uint32_t)modreduce<ALGO_SHOUP>(res, primeid_j);
                    }
                    else
                    {
                        ((uint64_t*)b[j_])[idx] = (uint64_t)modreduce<ALGO_SHOUP>(res, primeid_j);
                    }
                }
                else
                {
                    ...
                    for (int i_ = 0; i_ < n_d_n; ++i_)
                    {
                        ...
                        if constexpr (algo != 3)
                        {
                            uint64_t aux = modmult < algo > (buff[i_ * blockDim.x + tid],
                                G_->DecompAndModUp_matrix[MODUPIDX_MATRIX(
                                    n - 1, d, i_, primeid_j)], primeid_j);
                        ...
                        else
                        {
                            uint64_t aux = modmult < algo > (buff[i_ * blockDim.x + tid],
                                G_->DecompAndModUp_matrix[MODUPIDX_MATRIX(
                                    n - 1, d, i_, primeid_j)],
                                primeid_j,
                                G_->DecompAndModUp_matrix_shoup[MODUPIDX_MATRIX(
                                    n - 1, d, i_, primeid_j)]);
                            res = modadd(res, aux, primeid_j);
                        }
                        /*
...
#include "ntt_types.inc"
    ...
    void named_bar_sync(int barrier_id, int thread_count)
    {
    ...
    void named_bar_arrive(int barrier_id, int thread_count)
    {
    ...
    template <ALGO algo>
    __global__ void
    DecompAndModUpConv_spec(void** __restrict__ a, const int __grid_constant__ n, void** __restrict__ b,
                            const int __grid_constant__ d, const Global::Globals* Globals)
    {
        const int idx = blockIdx.x * blockDim.x + threadIdx.x;
        const int tid = threadIdx.x;
                ...
                if (pos < n_d_n)
                {
                    ...
                    if constexpr (algo != 3)
                    {
                        if (ISU64(primeid))
                        {
                            ...
                            buff[tid + blockDim.x * pos] = modmult < algo > (
                                /*((uint64_t*)(a[pos]))[idx]*/ buff[tid + blockDim.x * pos],
                                G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(
                                    d, n_d_n - 1, primeid)], primeid);
                        }
                        else
                    ...
                    else
                    {
                        if (ISU64(primeid))
                        {
                            ...
                            buff[tid + blockDim.x * pos] = modmult < algo > (
                                buff[tid + blockDim.x * pos] /*((uint64_t*)(a[pos]))[idx]
																								   */
                                ,
                                G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(d, n_d_n - 1, primeid)],
                                primeid,
                        ...
                        else
                        {
                            buff[tid + blockDim.x * pos] = modmult < algo > ((uint64_t)((uint32_t*)a[pos])[idx],
                                G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(
                                    d, n_d_n - 1, primeid)],
                                primeid,
                                G_->DecompAndModUp_pre_scale_shoup[
                                    MODUPIDX_SCALE(d, n_d_n - 1, primeid)]);
                        }
                    }
                }
        ...
        for (int j_ = threadIdx.y; j_ < C_.num_primeid_digit_to[d][n - 1]; j_ += blockDim.y)
        {
            ...
            if (primeid_j < n || primeid_j >= C_.L)
            {
                ...
                for (int i_ = 0; i_ < n_d_n; ++i_)
                {
                    ...
                    assert(MODUPIDX_MATRIX(n - 1, d, i_, primeid_j) < 64 * 64 * 64 * 8);
                    res = res + (__uint128_t)buff[i_ * blockDim.x + tid] * G_->DecompAndModUp_matrix[MODUPIDX_MATRIX(
                        n - 1, d, primeid /*i_*/, primeid_j)];
                }
...
#include "ntt_types.inc"
    ...
    template <ALGO algo>
    __global__ void
    DecompAndModUpConv_spec2(void** __restrict__ a, const int __grid_constant__ n, void** __restrict__ b,
                             const int __grid_constant__ d, const Global::Globals* Globals)
    {
            ...
            for (int i_ = 0; i_ < n_half; i_ += 1)
            {
                ...
                if (pos < n_d_n)
                {
                    ...
                    if constexpr (algo != 3)
                    {
                        ...
                        {
                            buff[tid + blockDim.x * pos] =
                                modmult < algo > (buff[tid + blockDim.x * pos],
                                    G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(d, n_d_n - 1, primeid)],
                                    primeid);
                        }
                        else
                ...
                if (pos < n_d_n)
                {
                    ...
                    if constexpr (algo != 3)
                    {
                        if (ISU64(primeid))
                        {
                            ulonglong2 data = ((ulonglong2*)buff)[tid + blockDim.x * pos];
                            data.x = modmult < algo > (
                                data.x, G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(d, n_d_n - 1, primeid)], primeid);
                            data.y = modmult < algo > (
                                data.y, G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(d, n_d_n - 1, primeid)], primeid);
                            ((ulonglong2*)buff)[tid + blockDim.x * pos] = data;
                        }
                        else
                        {
        ...
        for (int j_ = threadIdx.y; j_ < C_.num_primeid_digit_to[d][n - 1]; j_ += blockDim.y)
        {
            ...
            if (primeid_j < n || primeid_j >= C_.L)
            {
                ...
                for (int i_ = 0; i_ < n_d_n; ++i_)
                {
                    ...
                    ulonglong2 data = ((ulonglong2*)buff)[i_ * blockDim.x + tid];
                    assert(MODUPIDX_MATRIX(n - 1, d, i_, primeid_j) < 64 * 64 * 64 * 8);
                    resx = resx + (__uint128_t)data.x * G_->DecompAndModUp_matrix[MODUPIDX_MATRIX(
                        n - 1, d, primeid /*i_*/, primeid_j)];
                    resy = resy + (__uint128_t)data.y * G_->DecompAndModUp_matrix[MODUPIDX_MATRIX(
                        n - 1, d, primeid /*i_*/, primeid_j)];
                }
                ...
                if (!ISU64(primeid_j))
                {
                    ((uint32_t*)b[j_])[idx] = (uint32_t)modreduce<ALGO_SHOUP>(resx, primeid_j);
                }
                else
                {
                    ulonglong2 res = {modreduce<ALGO_SHOUP>(resx, primeid_j), modreduce<ALGO_SHOUP>(resy, primeid_j)};
                    ((ulonglong2*)b[j_])[idx] = res; //(uint64_t)modreduce<ALGO_SHOUP>(res, primeid_j);
                }
            }
        }
    }
                ...
                if (i_ + STAGES < n_half && pos + STAGES < n_d_n)
                {
                    ...
                    if (ISU64(primeid))
                    {
                        if (pos + STAGES < n_d_n)
                        {
                            ...
                            cuda::memcpy_async(
                                reinterpret_cast<ulonglong4*>(buff) + (blockDim.x * (pos + STAGES)) + tid,
                                reinterpret_cast<ulonglong4*>(peer_ptr) + (blockIdx.x * blockDim.x) + tid,
                                sizeof(ulonglong4),
                                pipe);
                        }
                    ...
                    if constexpr (algo != 3)
                    {
                        if (ISU64(primeid))
                        {
                            ...
                            data.y = modmult < algo > (
                                data.y, G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(d, n_d_n - 1, primeid)], primeid);
                            data.z = modmult < algo > (
                                data.z, G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(d, n_d_n - 1, primeid)], primeid);
                            data.w = modmult < algo > (
                                data.w, G_->DecompAndModUp_pre_scale[MODUPIDX_SCALE(d, n_d_n - 1, primeid)], primeid);
        ...
        for (int j_ = threadIdx.y; j_ < C_.num_primeid_digit_to[d][n - 1]; j_ += blockDim.y)
        {
            ...
            if (primeid_j < n || primeid_j >= C_.L)
            {
                ...
                for (int i_ = 0; i_ < n_d_n; ++i_)
                {
                    ...
                    resy = resy + (__uint128_t)data.y * G_->DecompAndModUp_matrix[MODUPIDX_MATRIX(
                        n - 1, d, primeid /*i_*/, primeid_j)];
                    resz = resz + (__uint128_t)data.z * G_->DecompAndModUp_matrix[MODUPIDX_MATRIX(
                        n - 1, d, primeid /*i_*/, primeid_j)];
                    resw = resw + (__uint128_t)data.w * G_->DecompAndModUp_matrix[MODUPIDX_MATRIX(
                        n - 1, d, primeid /*i_*/, primeid_j)];
                }
                ...
                {
                    ulonglong4 res = {
                        modreduce<ALGO_SHOUP>(resx, primeid_j),
                        modreduce<ALGO_SHOUP>(resy, primeid_j),
                        modreduce<ALGO_SHOUP>(resz, primeid_j),
                        modreduce<ALGO_SHOUP>(resw, primeid_j)
                    };
                    ((ulonglong4*)b[j_])[idx] = res; //(uint64_t)modreduce<ALGO_SHOUP>(res, primeid_j);
                }
            }
        }
    }
