...
template <typename T> class Limb {
  ...
  public:
    ...
    Limb(ContextData& context, T* data, const int offset, const int id, Stream& stream, const int primeid = -1, T* data_aux = nullptr, const int offset_aux = 0, int num_elem = -1);
    // void free();
