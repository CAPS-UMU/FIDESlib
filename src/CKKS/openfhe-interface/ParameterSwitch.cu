...
namespace FIDESlib
{
    namespace CKKS
    {
        ...
        {
            std::shared_ptr<lbcrypto::CryptoParametersCKKSRNS> init_param = std::dynamic_pointer_cast<
                lbcrypto::CryptoParametersCKKSRNS>(cc->GetCryptoParameters());
            ...
            parameters.SetSecretKeyDist(static_cast<uint32_t>(hamming_weight) == cc->GetRingDimension() / 2
        ...
        std::pair<std::pair<std::shared_ptr<lbcrypto::EvalKeyRelinImpl<lbcrypto::DCRTPoly>>, std::shared_ptr<
                                   ...
                                   int hamming_weight_b)
        {
            ...
            auto scaling = std::dynamic_pointer_cast<lbcrypto::CryptoParametersCKKSRNS>(cca->GetCryptoParameters())->
                GetScalingTechnique();
            ...
            else
            {
                ...
                saNew.SwitchModulusAtIndex(params.size() - 1, params.back()->GetModulus(),
                                           params.back()->GetRootOfUnity());
                ...
                sbNew.SwitchModulusAtIndex(params.size() - 1, params.back()->GetModulus(),
                                           params.back()->GetRootOfUnity());
