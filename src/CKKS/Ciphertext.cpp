...
inline bool AdjustForMultIsNoOp(const Ciphertext& a, const Ciphertext& b, RESCALE_TECHNIQUE st) {
    return a.getLevel() == b.getLevel() && (st == FIXEDMANUAL || (a.NoiseLevel == 1 && b.NoiseLevel == 1));
}
    ...
    if (cc.rescaleTechnique == FIXEDAUTO || cc.rescaleTechnique == FLEXIBLEAUTO || cc.rescaleTechnique == FLEXIBLEAUTOEXT) {
        ...
        if (!adjustForAddOrSub(b)) {
            ...
            if (b_.adjustForAddOrSub(*this))
                ...
                throwAdjustFailure("add", *this, b.c0.getLevel(), b.NoiseLevel, "ciphertext");
            return;
        }
    }
    ...
    if (cc.rescaleTechnique == FIXEDAUTO || cc.rescaleTechnique == FLEXIBLEAUTO || cc.rescaleTechnique == FLEXIBLEAUTOEXT) {
        ...
        if (!adjustForAddOrSub(b)) {
            if (b.adjustForAddOrSub(*this))
                ...
                throwAdjustFailure("addMutable", *this, b.c0.getLevel(), b.NoiseLevel, "ciphertext");
            return;
        }
    }
    ...
    if (cc.rescaleTechnique == FIXEDAUTO || cc.rescaleTechnique == FLEXIBLEAUTO || cc.rescaleTechnique == FLEXIBLEAUTOEXT) {
        ...
        if (!adjustForAddOrSub(b)) {
            ...
            if (b_.adjustForAddOrSub(*this))
                ...
                throwAdjustFailure("sub", *this, b.c0.getLevel(), b.NoiseLevel, "ciphertext");
            return;
        }
    }
...
void Ciphertext::subPtMutable(Plaintext& b) {
    ...
    if (cc.rescaleTechnique == FLEXIBLEAUTO || cc.rescaleTechnique == FLEXIBLEAUTOEXT || cc.rescaleTechnique == FIXEDAUTO) {
        ...
        if (!this->adjustCiphertextToPlaintext(b)) {
            if (!b.adjustPlaintextToCiphertext(b, *this)) {
                throwAdjustFailure("subPtMutable", *this, b.c0.getLevel(), b.NoiseLevel, "plaintext");
            } else {
                subPt(b);
            }
            return;
...
void Ciphertext::evalPartialLinearWSumWithBias(const std::vector<Ciphertext*>& ctxs, std::vector<double> weights, double bias, int limit, int target_level, int rescale) {
    CudaNvtxRange r(std::string{ sc::current().function_name() }.substr());
    CKKS::SetCurrentContext(cc_);
    op_count[OPS::WSUM]++;
...
Ciphertext::evalMultiplePartialLinearWSumWithBias(const std::vector<Ciphertext*>& ctxs, std::vector<std::vector<double>> weights, std::vector<int> target_level, int rescale) {
    CudaNvtxRange r(std::string{ sc::current().function_name() }.substr());
    Context cc_ = ctxs[0]->cc_;
    CKKS::SetCurrentContext(cc_);
    op_count[OPS::WSUM]++;
    ...
    std::sort(
        out_ctxt.begin(), out_ctxt.end(), [](const Objective& lhs, const Objective& rhs) { return lhs.n < rhs.n || (lhs.n == rhs.n && lhs.ctxt < rhs.ctxt); });
...
void Ciphertext::multMonomial(/*Ciphertext& ctxt,*/ int power) {
    ...
    if (!cc.precom.monomialCache.contains(power) || cc.precom.monomialCache.find(power)->second.getLevel() != this->getLevel()) {
        ...
        // Accessing `g.limb[i]` past the end would hand the kernel a garbage device pointer, and the
