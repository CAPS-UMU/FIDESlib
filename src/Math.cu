...
namespace FIDESlib
{
    ...
    std::vector<std::vector<uint64_t>> big_Q_prefix_mod_p(const std::vector<PrimeRecord>& q,
    ...
    std::vector<std::vector<uint64_t>> big_Q_hat(const std::vector<PrimeRecord>& p,
