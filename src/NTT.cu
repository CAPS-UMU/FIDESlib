...
namespace FIDESlib
{
                        ...
                        for (int k = 0; k < 2; ++k)
                        {
                            ...
                            else
                            {
                                ((T*)&aux)[k] = modmult<algo>(psi[hi_exp_br],
                                                              ((T*)G_->psi_no[primeid])[(lo_exp * 2) << (C_.logN -
                                                                  logN)],
                                                              primeid);
                            }
                        }
 ...
 __global__ void NTT_(const Global::Globals* Globals,
                      ...
                      T* __restrict__ res,
                      const T* __restrict__ pt,
                      const __grid_constant__int pt_elem,
                      ...
                      void** __restrict__ kskb)
    {
        ...
        if (ISU64(primeid))
        {
            NTT__<uint64_t, second, algo, mode>(Globals,
                                                (mode == NTT_RESCALE || mode == NTT_MULTPT) && !second
                                                    ? (uint64_t*)dat[0]
