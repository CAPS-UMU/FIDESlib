...
__device__ __forceinline__
    ...
    void
    forward_negacyclic_scale(char* buffer, const int primeid, T* psi, T* psi_shoup, const Global::Globals* Globals, const int logn) {
                ...
                for (int i = 0; i < M; i += 1) {
...

                    {
                        ...
                        for (int k = 0; k < 2; ++k) {
                            ...
                            if constexpr (algo == 3) {
                                ((T*)&aux)[k] =
                                    modmult<algo>(((T*)G_->psi_no[primeid])[(lo_exp * 2) << (C_.logN - logN)], psi[hi_exp_br], primeid, psi_barret[hi_exp_br]);
                            } else {
                                ((T*)&aux)[k] = modmult<algo>(psi[hi_exp_br], ((T*)G_->psi_no[primeid])[(lo_exp * 2) << (C_.logN - logN)], primeid);
                            }
                        }
                    }
