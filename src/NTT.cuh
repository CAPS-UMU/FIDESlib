...
namespace FIDESlib
{
    ...
    template <uint32_t M, bool second>
    uint32_t NTT_block_dim_X(int logN)
    {
        return second
    ...
    template <uint32_t M, bool second>
    uint32_t NTT_grid_dim_X(int logN)
    {
    ...
    template <uint32_t M, bool second>
    uint32_t INTT_block_dim_X(int logN)
    {
        return second
                   ? (uint32_t)(1 << ((logN + 1 + (logN > 13 ? 0 : 0)) / 2 - 1))
                   : (uint32_t)(1 << ((logN + (logN > 13 ? 0 : 0)) / 2 - 1));
    ...
    template <uint32_t M, bool second>
    uint32_t INTT_grid_dim_X(int logN)
    {
        uint32_t blockDimX = INTT_block_dim_X<M, second>(logN);
        return second ? (1 << logN) / blockDimX / 2 / M : (1 << logN) / blockDimX / 2 / M;
    }

    template <uint32_t M, typename T, ALGO algo>
    uint32_t NTT_shmem(uint32_t blockDimX)
    {
 ...
 __global__ void INTT_(const Global::Globals* Globals,
    ...
    template <typename T, ALGO algo = ALGO_SHOUP, INTT_MODE mode = INTT_NONE>
 __global__ void INTT_1D_(const Global::Globals* Globals,
                          T* __restrict__ dat,
                          const int __grid_constant__ primeid,
                          T* __restrict__ res,
                          const T* __restrict__ dat2 = nullptr,
 ...
 __global__ void NTT_(const Global::Globals* Globals,
                      ...
                      const __grid_constant__int pt_elem = -1,
                      const int __grid_constant__ primeid_rescale = -1,
                      void** __restrict__ res2 = nullptr,
                      void** __restrict__ kskb = nullptr);

    template <typename T, ALGO algo = ALGO_SHOUP, NTT_MODE mode = NTT_NONE>
 __global__ void NTT_1D_(const Global::Globals* Globals,
                         T* __restrict__ dat,
                         const int __grid_constant__ primeid,
                         T* __restrict__ res,
                         const T* __restrict__ pt = nullptr,
