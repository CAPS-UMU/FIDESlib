...
#define ASSERT_ERROR_OK(result, resultGPU)                                                                                                             \
    do {                                                                                                                                               \
        ...
        std::cout << "Max error: " << err_max_ << " (Expected: " << pow(2.0, -err_cpu_->GetLogPrecision() + 1) << "), dev: " << err_acc_ << std::endl; \
...
#define ASSERT_EQ_DCRTPOLY(ct1, ct2)                                                                                                               \
    do {                                                                                                                                           \
        ...
        for (size_t eqp_tower_ = 0; eqp_tower_ < eqp_p1_->m_vectors.size(); ++eqp_tower_) {                                                        \
            ASSERT_EQ(eqp_p1_->m_vectors[eqp_tower_].m_values.get()->m_data.size(), eqp_p2_->m_vectors[eqp_tower_].m_values.get()->m_data.size()); \
            ASSERT_EQ(eqp_p1_->m_vectors[eqp_tower_].m_values.get()->m_data, eqp_p2_->m_vectors[eqp_tower_].m_values.get()->m_data);               \
        }                                                                                                                                          \
    } while (0);
