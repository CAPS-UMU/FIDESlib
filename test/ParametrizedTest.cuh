 ...
 * Todo: Fix prime sizes.
 */
inline GeneralTestParams gparams64_16{ .multDepth = 29, .firstModSize = 60, .scaleModSize = 59 /*35 fails*/, .batchSize = 8, .ringDim = 1 << 16, .dnum = 4,
                                       .GPUs = devices };
...
inline GeneralTestParams gparams64_16_boot1{ .multDepth = 23, .firstModSize = 52, .scaleModSize = 48 /*35 fails*/, .batchSize = 8, .ringDim = 1 << 16,
                                             .dnum = 4, .GPUs = devices };
...
class GeneralParametrizedTest
	: public testing::TestWithParam<std::tuple<std::tuple<GeneralTestParams, FIDESlib::CKKS::Parameters>, lbcrypto::ScalingTechnique>> {
protected:
...
#define ASSERT_EQ_CIPHERTEXT(ct1, ct2)                                                                                                    \
	do {                                                                                                                                  \
		...
		for (size_t eq_elem_ = 1; eq_elem_ < 2; --eq_elem_) {                                                                             \
			const auto& eq_e1_ = eq_ct1_.get()->GetElements().at(eq_elem_).GetAllElements();                                              \
			const auto& eq_e2_ = eq_ct2_.get()->GetElements().at(eq_elem_).GetAllElements();                                              \
			ASSERT_EQ(eq_e1_.size(), eq_e2_.size());                                                                                      \
