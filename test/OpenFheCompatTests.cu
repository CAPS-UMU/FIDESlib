...
TEST(OpenFHECompatTests, EvalBootstrapSparseEncaps) {
    uint32_t multDepth = 25;
    uint32_t scaleModSize = 50;
    uint32_t batchSize = 8;
...
// its disjoint FIDESlib keys at 2N-6/2N-8. They assert value-level agreement first and
// bit-exactness second. The UNIFORM_TERNARY / SPARSE_TERNARY rows must match — if one of
// those fails, that is a different bug (e.g. a bootK regression).
